...
import mindtorch.distributed.fsdp._traversal_utils as traversal_utils
import mindtorch.nn as nn
from mindtorch.distributed.algorithms._checkpoint.checkpoint_wrapper import (
    _CHECKPOINT_WRAPPED_MODULE,
    ActivationWrapper,
)
from mindtorch.distributed.algorithms._comm_hooks import LOW_PRECISION_HOOKS
from mindtorch.distributed.fsdp._common_utils import (
    _FSDPState,
    _get_param_to_fqns,
    FSDP_PREFIX,
    FSDP_WRAPPED_MODULE,
...
from mindtorch.distributed.fsdp._dynamo_utils import _annotate_modules_for_dynamo
from mindtorch.distributed.fsdp._init_utils import (
...
from mindtorch.distributed.fsdp._runtime_utils import (
...
from mindtorch.distributed.fsdp._wrap_utils import _auto_wrap
from mindtorch.distributed.fsdp.api import (
    BackwardPrefetch,
    CPUOffload,
...
from mindtorch.distributed.tensor import DeviceMesh
from mindtorch.distributed.utils import _p_assert
...
class FullyShardedDataParallel(nn.Module, _FSDPState):
    ...
    Example::
        ...
        >>> import mindtorch
        >>> from mindtorch.distributed.fsdp import FullyShardedDataParallel as FSDP
        >>> mindtorch.cuda.set_device(device_id)
        >>> sharded_module = FSDP(my_module)
        >>> optim = mindtorch.optim.Adam(sharded_module.parameters(), lr=0.0001)
        >>> x = sharded_module(x, y=3, z=mindtorch.Tensor([1]))
        >>> loss = x.sum()
        >>> loss.backward()
        >>> optim.step()
    ...
    * Set the device using ``mindtorch.cuda.set_device(dev_id)``
    * Pass ``dev_id`` into the ``device_id`` constructor argument.
    ...
    FSDP replaces managed modules' parameters with ``mindtorch.Tensor``
    ...
    * You should avoid modifying the parameters between forward and
    ...
    Args:
        ...
        auto_wrap_policy (Optional[Union[Callable[[nn.Module, bool, int], bool], ModuleWrapPolicy, CustomPolicy]]):
            ...
            ``mindtorch.distributed.fsdp.wrap.py`` gives an example callable that
        ...
        ignored_modules (Optional[Iterable[mindtorch.nn.Module]]): Modules whose
        ...
        param_init_fn (Optional[Callable[[nn.Module], None]]):
            A ``Callable[mindtorch.nn.Module] -> None`` that
        ...
        device_id (Optional[Union[int, mindtorch.device]]): An ``int`` or
            ``mindtorch.device`` giving the CUDA device on which FSDP
            ...
            default CUDA device was set (e.g. via ``mindtorch.cuda.set_device``),
            then the user may pass ``mindtorch.cuda.current_device`` to this.
            (Default: ``None``)
        ...
        use_orig_params (bool): Setting this to ``True`` has FSDP use
            ...
            use ``mindtorch.compile()``. Setting this to ``False`` exposes FSDP's
            internal :class:`FlatParameter` s to the user via
            :meth:`nn.Module.named_parameters`. (Default: ``False``)
        ignored_states (Optional[Iterable[mindtorch.nn.Parameter]], Optional[Iterable[mindtorch.nn.Module]]):
        ...
        device_mesh (Optional[DeviceMesh]): DeviceMesh can be used as an altenative to
            ...
            https://pymindtorch.org/tutorials/recipes/distributed_device_mesh.html
    """
    ...
    def __init__(
        ...
        ignored_modules: Optional[Iterable[mindtorch.nn.Module]] = None,
        param_init_fn: Optional[Callable[[nn.Module], None]] = None,
        device_id: Optional[Union[int, mindtorch.device]] = None,
        sync_module_states: bool = False,
        forward_prefetch: bool = False,
        limit_all_gathers: bool = True,
        use_orig_params: bool = False,
        ignored_states: Union[
            Optional[Iterable[mindtorch.nn.Parameter]], Optional[Iterable[mindtorch.nn.Module]]
        ] = None,
        device_mesh: Optional[DeviceMesh] = None,
    ):
        mindtorch._C._log_api_usage_once("mindtorch.distributed.fsdp")
        super().__init__()
    ...
    def fsdp_modules(
        ...
        Args:
            module (mindtorch.nn.Module): Root module, which may or may not be an
                ``FSDP`` module.
    ...
    def apply(self, fn: Callable[[nn.Module], None]) -> "FullyShardedDataParallel":
        ...
        Compared to ``mindtorch.nn.Module.apply``, this version additionally gathers
    ...
    def set_state_dict_type(
        ...
        Args:
            module (mindtorch.nn.Module): Root module.
        ...
        warnings.warn(
            ...
            "API doc: https://pymindtorch.org/docs/stable/distributed.checkpoint.html"
            "#mindtorch.distributed.checkpoint.state_dict.get_state_dict ."
            "Tutorial: https://pymindtorch.org/tutorials/recipes/distributed_checkpoint_recipe.html .",
            FutureWarning,
        )
    ...
    def forward(self, *args: Any, **kwargs: Any) -> Any:
        ...
        handle = self._handle
        with mindtorch.autograd.profiler.record_function(
    ...
    def named_buffers(
        ...
        *args,
        **kwargs,
    ) -> Iterator[Tuple[str, mindtorch.Tensor]]:
    ...
    def named_parameters(
        ...
        *args,
        **kwargs,
    ) -> Iterator[Tuple[str, mindtorch.nn.Parameter]]:
    ...
    def clip_grad_norm_(
        ...
        if self._zero_scalar is None:
            self._zero_scalar = mindtorch.tensor(0.0, device=self.compute_device)
        self._assert_state(TrainingState.IDLE)
        ...
        if all_no_shard:
            return mindtorch.nn.utils.clip_grad_norm_(
        ...
        if norm_type == math.inf:
            total_norm = (
                mindtorch.maximum(local_sharded_norm, local_nonsharded_norm)
                if local_nonsharded_norm is not None
                else local_sharded_norm
            )
            dist.all_reduce(
                total_norm, op=mindtorch.distributed.ReduceOp.MAX, group=self.process_group
            )
        else:
        ...
        # `if clip_coef < 1`
        clip_coef_clamped = mindtorch.clamp(clip_coef, max=1.0)
        for grad in grads:
    ...
    def _optim_state_dict_impl(
        model: mindtorch.nn.Module,
        optim: mindtorch.optim.Optimizer,
        optim_state_dict: Dict[str, Any],
        optim_input: Optional[
            Union[
                List[Dict[str, Any]],
                Iterable[mindtorch.nn.Parameter],
            ]
        ] = None,
    ...
    def _optim_state_dict_to_load_impl(
        ...
        optim: Optional[mindtorch.optim.Optimizer] = None,
        full_state_dict: bool = True,
        rank0_only: bool = False,
    ...
    def full_optim_state_dict(
        ...
        :meth:`mindtorch.optim.Optimizer.state_dict`, i.e. with keys ``"state"``
        ...
        Unlike ``mindtorch.optim.Optimizer.state_dict()``, this method
        uses full parameter names as keys instead of parameter IDs.

        Like in :meth:`mindtorch.optim.Optimizer.state_dict`, the tensors
        ...
        Args:
            model (mindtorch.nn.Module): Root module (which may or may not be a
                :class:`FullyShardedDataParallel` instance) whose parameters
                were passed into the optimizer ``optim``.
            optim (mindtorch.optim.Optimizer): Optimizer for ``model`` 's
                parameters.
            optim_input (Optional[Union[List[Dict[str, Any]], Iterable[mindtorch.nn.Parameter]]]):
        ...
        Returns:
            ...
            :meth:`mindtorch.optim.Optimizer.state_dict`. If ``rank0_only=True``,
    ...
    def shard_full_optim_state_dict(
        ...
        Example::
            ...
            >>> mindtorch.save(full_osd, PATH)
            >>> # Define new model with possibly different world size
            >>> new_model, new_optim = ...
            >>> full_osd = mindtorch.load(PATH)
        ...
        Args:
            ...
            optim_input (Optional[Union[List[Dict[str, Any]], Iterable[mindtorch.nn.Parameter]]]):
                ...
                ``model.parameters()``. This argument is deprecated, and there
                is no need to pass it in anymore. (Default: ``None``)
            optim (Optional[mindtorch.optim.Optimizer]): Optimizer that will load
    ...
    def optim_state_dict(
        ...
        Example::
            ...
            >>> # xdoctest: +SKIP("undefined variables")
            >>> from mindtorch.distributed.fsdp import FullyShardedDataParallel as FSDP
            >>> from mindtorch.distributed.fsdp import StateDictType
            >>> from mindtorch.distributed.fsdp import FullStateDictConfig
            >>> from mindtorch.distributed.fsdp import FullOptimStateDictConfig
            >>> # Save a checkpoint
            >>> model, optim = ...
            >>> FSDP.set_state_dict_type(
            >>>     model,
    ...
    def optim_state_dict_to_load(
        ...
        Args:
            ...
            is_named_optimizer (bool): Is this optimizer a NamedOptimizer or
                ...
                KeyedOptimizer or mindtorch.distributed's NamedOptimizer.
    ...
    def register_comm_hook(self, state: object, hook: callable):
        ...
        Args:
            ...
            hook (Callable): Callable, which has one of the following signatures:
                            1) ``hook: Callable[mindtorch.Tensor] -> None``:
                            This function takes in a Python tensor, which represents
                            the full, flattened, unsharded gradient with respect to all variables
                            corresponding to the model this FSDP unit is wrapping
                            (that are not wrapped by other FSDP sub-units).
                            It then performs all necessary processing and returns ``None``;
                            2) ``hook: Callable[mindtorch.Tensor, mindtorch.Tensor] -> None``:
...
def _get_grad_norm(
    ...
    # single vector
    grad_norm = mindtorch.linalg.vector_norm(
        mindtorch.stack(
            [
                mindtorch.linalg.vector_norm(grad.detach(), norm_type, dtype=mindtorch.float32)
                for grad in grads
            ],
        ),
        norm_type,
...
def _get_param_to_fqn(
    model: mindtorch.nn.Module,
) -> Dict[mindtorch.nn.Parameter, str]:
    """
    ...
    Args:
        model (mindtorch.nn.Module): Root module, which should not contain any
...
def _get_fqn_to_param(
    model: mindtorch.nn.Module,
) -> Dict[str, mindtorch.nn.Parameter]:
