...
import mindtorch
from mindtorch._subclasses.fake_tensor import FakeTensor
from mindtorch.distributed.tensor import DeviceMesh, distribute_tensor, DTensor
...
from mindtorch.distributed.tensor.parallel.style import ColwiseParallel, ParallelStyle
from mindtorch.distributed.tensor.placement_types import Placement, Replicate, Shard
from mindtorch.export import ExportedProgram
from mindtorch.export.exported_program import ExportGraphSignature
from mindtorch.fx import GraphModule
from mindtorch.fx.experimental.proxy_tensor import make_fx
from mindtorch.fx.node import Node
from mindtorch.fx.passes.infra.pass_base import PassBase, PassResult
from mindtorch.fx.passes.shape_prop import _extract_tensor_metadata
...
class _TensorParallelTransformPass(PassBase):
    ...
    def __init__(
        ...
        world_size: int,
        device_type: str,
        state_dict: Dict[str, mindtorch.Tensor],
        graph_signature: ExportGraphSignature,
        parallel_strategies: Dict[str, ParallelStyle],
    ) -> None:
        super().__init__()
        self.rank = rank
        self.mesh = DeviceMesh(device_type, mindtorch.arange(world_size))
        self.state_dict: Dict[str, mindtorch.Tensor] = state_dict
...
def _partitioner(gm: mindtorch.fx.GraphModule) -> mindtorch.fx.GraphModule:
    """
...
def _insert_reshard_gm(
    ...
    def reshard_fn(local_tensor: mindtorch.Tensor) -> mindtorch.Tensor:
...
def _clean_up_graph_metadata(gm: mindtorch.fx.GraphModule) -> None:
    """
    ...
    for node in gm.graph.nodes:
        ...
        if "val" in node.meta and isinstance(node.meta["val"], mindtorch.Tensor):
...
def _get_op_schema(
    ...
    op_schema = OpSchema(
        op=cast(mindtorch._ops.OpOverload, node.target),
...
def _shard_state_dict(
    ...
    for node, placement_strategy in placement_strategies.items():
        ...
        state_dict[fqn] = (
            mindtorch.nn.Parameter(local_param)
            if isinstance(original_param, mindtorch.nn.Parameter)
            else local_param
        )
