...
# import mindtorch.utils.data.graph_settings
from mindtorch._utils import ExceptionWrapper
from mindtorch.utils.data import _utils
# from mindtorch.utils.data.datapipes.datapipe import (
#     _IterDataPipeSerializationWrapper,
#     _MapDataPipeSerializationWrapper,
#     IterDataPipe,
#     MapDataPipe,
# )
from mindtorch.utils.data.dataset import Dataset, IterableDataset
from mindtorch.utils.data.sampler import (
    BatchSampler,
    RandomSampler,
    Sampler,
    SequentialSampler,
...
# (one has to explicitly directly `import mindtorch.utils.data.dataloader`), there
...
class _InfiniteConstantSampler(Sampler):
    ...
    Used as sampler for :class:`~mindtorch.utils.data.IterableDataset`.
    """
...
def _sharding_worker_init_fn(worker_init_fn, world_size, rank_id, worker_id):
    ...
    info = mindtorch.utils.data.get_worker_info()
    ...
    mindtorch.utils.data.graph_settings.apply_sharding(
...
def _share_dist_seed(generator, pg):
    _shared_seed = mindtorch.empty((), dtype=mindtorch.int64).random_(generator=generator)
...
class DataLoader(Generic[_T_co]):
    ...
    The :class:`~mindtorch.utils.data.DataLoader` supports both map-style and
    ...
    Args:
        ...
        generator (mindtorch.Generator, optional): If not ``None``, this RNG will be used
    ...
    .. warning:: ``len(dataloader)`` heuristic is based on the length of the sampler used.
                 When :attr:`dataset` is an :class:`~mindtorch.utils.data.IterableDataset`,
                 ...
                 :class:`~mindtorch.utils.data.IterableDataset` interacts with
    ...
    def __init__(
        ...
        *,
        ...
        # mindtorch._C._log_api_usage_once("python.data_loader")
        ...
        if isinstance(dataset, IterableDataset):
            ...
            #         dataset = mindtorch.utils.data.graph_settings.apply_shuffle_settings(
        ...
        # mindtorch.set_vital("Dataloader", "enabled", "True")  # type: ignore[attr-defined]
        ...
        if multiprocessing_context is not None:
            if self.num_workers > 0:
                if isinstance(multiprocessing_context, str):
                    valid_start_methods = mindtorch.multiprocessing.get_all_start_methods()
                    ...
                    multiprocessing_context = mindtorch.multiprocessing.get_context(
    ...
    def check_worker_number_rationality(self):
        ...
        #        other than `mindtorch.set_num_threads` to 1 in the worker process, if the passing
...
class _BaseDataLoaderIter:
    def __init__(self, loader: DataLoader) -> None:
        ...
        #     shared_rng = mindtorch.Generator()
        #     shared_rng.manual_seed(self._shared_seed)
        #     self._dataset = mindtorch.utils.data.graph_settings.apply_random_seed(
        ...
        if len(loader.pin_memory_device) == 0:
            self._pin_memory = loader.pin_memory and mindtorch.cuda.is_available()
        ...
        self._base_seed = (
            mindtorch.empty((), dtype=mindtorch.int64)
    ...
    def __next__(self) -> Any:
        # with mindtorch.autograd.profiler.record_function(self._profile_name):
        ...
        if (
            ...
            if self._num_workers > 0:
                warn_msg += (
                    ...
                    "https://pymindtorch.org/docs/stable/data.html#mindtorch.utils.data.IterableDataset for examples."
                )
            warnings.warn(warn_msg)
        return data
...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        if loader.multiprocessing_context is None:
            multiprocessing_context = mindtorch.multiprocessing
        else:
        ...
        if self._pin_memory:
            ...
            if self._pin_memory_device == "xpu":
                current_device = mindtorch.xpu.current_device()  # type: ignore[attr-defined]
            elif self._pin_memory_device == mindtorch._C._get_privateuse1_backend_name():
                custom_device_mod = getattr(
                    torch, mindtorch._C._get_privateuse1_backend_name()
                )
                current_device = custom_device_mod.current_device()
            else:
                current_device = mindtorch.cuda.current_device()  # choose cuda for default
            ...
            try:
                ...
                if e.errno == errno.EMFILE:
                    raise RuntimeError(
                        ...
                        " `mindtorch.multiprocessing.set_sharing_strategy('file_system')`"
    ...
    # `mindtorch.multiprocessing`) raises a `RuntimeError: received 0 items of ancdata`
    #
