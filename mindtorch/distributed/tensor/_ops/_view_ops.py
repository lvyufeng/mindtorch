...
from mindtorch.distributed.tensor._ops.utils import (
...
from mindtorch.distributed.tensor.placement_types import Placement, Replicate, Shard
...
dim_maps: Dict[Callable[..., mindtorch.Tensor], Callable[..., DimMap]] = {
    mindtorch.atleast_1d: lambda x: dim_pad_left(x.ndim, 1),
    mindtorch.atleast_2d: lambda x: dim_pad_left(x.ndim, 2),
    mindtorch.atleast_3d: lambda x: dim_atleast_3d(x.ndim),
    mindtorch.broadcast_to: lambda input, shape: expand(input.shape, shape),
    Tensor.expand: lambda self, *sizes: expand(self.shape, normalize_sizes(sizes)),
    mindtorch.flatten: lambda tensor: dim_flatten(tensor.ndim),
    mindtorch.movedim: lambda input, source, destination: dim_movedim(
        input.ndim, source, destination
    ),
    mindtorch.permute: lambda input, dims: tuple(
        InputDim(i) for i in normalize_dims(dims, input.ndim)
    ),
    mindtorch.ravel: lambda tensor: dim_flatten(tensor.ndim),
    Tensor.repeat: lambda self, *sizes: dim_repeat(self.ndim, sizes),
    mindtorch.reshape: lambda input, shape: view_groups(input.shape, shape),
    mindtorch.squeeze: lambda input, dim=None: dim_squeeze(input.shape, dim),
    mindtorch.tile: lambda input, dims: dim_tile(input.ndim, dims),
    mindtorch.transpose: lambda input, dim0, dim1: dim_transpose(input.ndim, dim0, dim1),
    mindtorch.unsqueeze: lambda input, dim: dim_unsqueeze(input.ndim, dim),
    Tensor.view: lambda input, *shape: view_groups(input.shape, shape),
    mindtorch.view_as_complex: lambda input: dim_flatten(input.ndim, input.ndim - 2),
    mindtorch.view_as_real: lambda input: dim_view_as_real(input.shape),
}
...
def register_op_strategy_map(
    aten_op_overload: mindtorch._ops.OpOverload,
    local_op_name: Callable[..., mindtorch.Tensor],
...
register_op_strategy_map(aten.squeeze.default, mindtorch.squeeze)
register_op_strategy_map(
    aten.squeeze.dim, mindtorch.squeeze, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.view.default, Tensor.view, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.reshape.default, mindtorch.reshape, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten._unsafe_view.default, Tensor.view, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.unsqueeze.default, mindtorch.unsqueeze, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.expand.default, Tensor.expand, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.permute.default, mindtorch.permute, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.repeat.default, Tensor.repeat, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(
    aten.transpose.int, mindtorch.transpose, schema_info=RuntimeSchemaInfo(1)
)
register_op_strategy_map(aten.view_as_complex.default, mindtorch.view_as_complex)
register_op_strategy_map(aten.view_as_real.default, mindtorch.view_as_real)
