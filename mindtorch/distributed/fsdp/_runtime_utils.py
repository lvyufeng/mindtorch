...
from mindtorch.autograd import Variable
from mindtorch.autograd.graph import register_multi_grad_hook
...
from mindtorch.distributed.fsdp._init_utils import HYBRID_SHARDING_STRATEGIES
from mindtorch.distributed.fsdp.api import BackwardPrefetch
...
from mindtorch.utils import _pytree as pytree
...
def _share_state_and_init_handle_attrs(
    ...
    for handle in root_state._all_handles:
        ...
        if handle._has_optim_in_backward:
            mindtorch._C._log_api_usage_once("fsdp.optimizer_in_backward")
...
def _unshard(
    ...
    unshard_stream: mindtorch.Stream,
    pre_unshard_stream: mindtorch.Stream,
) -> None:
    """
    ...
    if state.limit_all_gathers:
        ...
        if event:
            with mindtorch.profiler.record_function(
...
def _pre_forward(
    ...
    """
    with mindtorch.profiler.record_function("FullyShardedDataParallel._pre_forward"):
        ...
        if handle and handle._offload_params and handle.flat_param._cpu_grad is None:
            handle.flat_param._cpu_grad = mindtorch.zeros_like(
                handle.flat_param._local_shard, device=mindtorch.device("cpu")
        ...
        if should_cast_forward_inputs and state.mixed_precision.cast_forward_inputs:
            ...
            input_dtype: Optional[mindtorch.dtype] = state.mixed_precision.param_dtype
...
def _post_forward(
    ...
    parameter.
    """
    with mindtorch.profiler.record_function("FullyShardedDataParallel._post_forward"):
...
def _root_pre_forward(
    ...
    """
    with mindtorch.profiler.record_function("FullyShardedDataParallel._root_pre_forward"):
        _lazy_init(state, module)
        ...
        with mindtorch.profiler.record_function("FullyShardedDataParallel._to_kwargs"):
...
def _root_cast_forward_input(
    state: _FSDPState, module: mindtorch.nn.Module, args, kwargs
) -> Tuple[Any, Any]:
    if state._handle:
...
def _pre_backward_hook(
    ...
    *unused: Any,
    ...
    with mindtorch.profiler.record_function("FullyShardedDataParallel._pre_backward_hook"):
...
def _get_reduce_scatter_tensors(
    state: _FSDPState, unsharded_grad: mindtorch.Tensor
    ...
    new_sharded_grad = mindtorch.empty_like(chunks[0])  # padded
...
def _post_reduce_grad_callback(
    ...
    grad_to_offload: mindtorch.Tensor,
):
    """
...
def _div_if_needed(tensor: mindtorch.Tensor, div_factor: float) -> None:
    if div_factor > 1:
...
def _check_grad_to_accumulate(
    new_sharded_grad: mindtorch.Tensor,
    accumulated_grad: mindtorch.Tensor,
) -> None:
    _p_assert(
...
def _finalize_params(
    ...
    if not handle:
        return
    flat_param = handle.flat_param
    if mindtorch.distributed._functional_collectives.is_torchdynamo_compiling():
...
def _register_pre_backward_hooks(
    ...
    def _register_hook(t: mindtorch.Tensor) -> mindtorch.Tensor:
        if t.requires_grad:
            t.register_hook(
                mindtorch.utils.hooks.unserializable_hook(
...
def _register_post_backward_reshard_only_hook(
    ...
    inp_tensors: Optional[List[mindtorch.Tensor]] = None
    if not handle:
        return
    flat_param = handle.flat_param
    ...
    if inp_tensors is None:
        ...
        inp_tensors = [
            obj for obj in args_flat if mindtorch.is_tensor(obj) and obj.requires_grad
        ]
...
def _wait_for_computation_stream(
    computation_stream: mindtorch.Stream,
    unshard_stream: mindtorch.Stream,
    pre_unshard_stream: mindtorch.Stream,
):
    """
...
def _get_buffers_and_dtypes_for_computation(
    state: _FSDPState,
    root_module: nn.Module,
) -> Tuple[List[mindtorch.Tensor], List[Optional[mindtorch.dtype]]]:
    """
    ...
    buffer_dtypes: List[Optional[mindtorch.dtype]] = []
    visited_buffers: Set[mindtorch.Tensor] = set()
...
def _get_orig_buffer_dtypes(
    ...
    """
    buffer_dtypes: List[mindtorch.dtype] = []
...
def _cast_buffers_to_dtype_and_device(
    ...
    for buffer, buffer_dtype in zip(buffers, buffer_dtypes):
        if not mindtorch.is_floating_point(buffer) or buffer_dtype is None:
