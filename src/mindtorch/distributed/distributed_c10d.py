...
from mindtorch.utils._typing_utils import not_none
...
try:
    from mindtorch._C._distributed_c10d import ProcessGroupMPI

    ProcessGroupMPI.__module__ = "mindtorch.distributed.distributed_c10d"
    __all__ += ["ProcessGroupMPI"]
except ImportError:
    _MPI_AVAILABLE = False

try:
    from mindtorch._C._distributed_c10d import ProcessGroupNCCL

    ProcessGroupNCCL.__module__ = "mindtorch.distributed.distributed_c10d"
    __all__ += ["ProcessGroupNCCL"]
except ImportError:
    _NCCL_AVAILABLE = False

try:
    from mindtorch._C._distributed_c10d import _ProcessGroupWrapper, ProcessGroupGloo

    ProcessGroupGloo.__module__ = "mindtorch.distributed.distributed_c10d"
    __all__ += ["ProcessGroupGloo"]
except ImportError:
    _GLOO_AVAILABLE = False

try:
    from mindtorch._C._distributed_c10d import ProcessGroupUCC

    ProcessGroupUCC.__module__ = "mindtorch.distributed.distributed_c10d"
...
# complex tensors as real (mindtorch.view_as_real), meaning that calling
            ...
            for device, default_backend in Backend.default_device_backend_map.items():
                if is_backend_available(default_backend):
                    if (
                        ...
                        and not mindtorch.cuda.is_available()
                    ):
                        continue
...
class P2POp:
    ...
    Args:
        op (Callable): A function to send data to or receive data from a peer process.
            The type of ``op`` is either ``mindtorch.distributed.isend`` or
            ``mindtorch.distributed.irecv``.
...
class _CollOp:
    ...
    Args:
        op (Callable): A collective function, e.g. ``mindtorch.distributed.all_reduce``.
    ...
    def __init__(
        ...
        op: Callable,
        tensor: mindtorch.Tensor,
        dst_tensor: Optional[mindtorch.Tensor] = None,
...
def _get_object_coll_device(group: Optional[ProcessGroup] = None) -> str:
    ...
    if len(devices) == 1:
        ...
        return "cpu"
    elif mindtorch.device("cpu") in devices:
...
def get_group_rank(group: ProcessGroup, global_rank: int) -> int:
    ...
    if group not in _world.pg_group_ranks:
        raise ValueError(
            f"Group {group} is not registered, please create group with mindtorch.distributed.new_group API"
        )
...
@deprecated(
    "`mindtorch.distributed.distributed_c10d._get_global_rank` is deprecated, "
    "please use `mindtorch.distributed.distributed_c10d.get_global_rank` instead",
...
def _check_single_tensor(param, param_name) -> None:
    ...
    if not isinstance(param, mindtorch.Tensor):
        raise TypeError(
            f"""Invalid function argument. Expected parameter `{param_name}` of type mindtorch.Tensor
...
def _check_tensor_list(param, param_name) -> None:
    ...
    if not isinstance(param, list):
        raise TypeError(
            f"""Invalid function argument. Expected parameter `{param_name}` of type List[mindtorch.Tensor]
             but got {type(param)} instead."""
        )
    elif not all(isinstance(p, mindtorch.Tensor) for p in param):
        raise TypeError(
            f"""Invalid function argument. Expected parameter `{param_name}` of type List[mindtorch.Tensor]
...
def _ensure_all_tensors_same_dtype(*tensors) -> None:
    ...
    for tensor in itertools.chain.from_iterable(map(_as_iterable, tensors)):
        ...
        #     tensor_dtype = (
        #         mindtorch.float32 if tensor_dtype == mindtorch.complex64 else mindtorch.complex128
        #     )
...
def _check_op(op) -> None:
    ...
    if op not in [isend, irecv]:
        raise ValueError(
            ...
            "to be of type ``mindtorch.distributed.isend`` or "
            "``mindtorch.distributed.irecv``."
        )
...
def _check_p2p_op_list(p2p_op_list) -> None:
    ...
    if not isinstance(p2p_op_list, list) or not all(
        ...
        raise ValueError(
            ...
            "to be of type ``mindtorch.distributed.P2POp``."
        )
...
def is_backend_available(backend: str) -> bool:
    ...
    available_func = getattr(mindtorch.distributed, f"is_{backend.lower()}_available", None)
    if available_func:
...
def is_torchelastic_launched() -> bool:
    """
    Check whether this process was launched with ``mindtorch.distributed.elastic`` (aka torchelastic).
...
def _update_default_pg(pg) -> None:
    ...
    # mindtorch._C._distributed_c10d._set_global_rank(rank)
...
def _get_process_group_uid(pg: ProcessGroup) -> int:
    backend = None
    try:
        backend = pg._get_backend(mindtorch.device("cuda"))
    except RuntimeError:
        pass
...
def _add_ephemeral_timeout_for_all_pgs(timeout: timedelta) -> None:
    ...
    for pg in _world.pg_map.keys():
        ...
        if mindtorch.device("cuda") in devices:
            backend = pg._get_backend(mindtorch.device("cuda"))
            if is_nccl_available() and isinstance(backend, ProcessGroupNCCL):
...
def _set_pg_timeout(timeout: timedelta, group: Optional[ProcessGroup] = None) -> None:
    ...
    devices = group._device_types
    backends = set()
    if mindtorch.device("cpu") in devices and is_gloo_available():
        backend = group._get_backend(mindtorch.device("cpu"))
        if isinstance(backend, ProcessGroupGloo):
            backends.add(backend)
    if mindtorch.device("cuda") in devices:
        backend = group._get_backend(mindtorch.device("cuda"))
        if is_nccl_available() and isinstance(backend, ProcessGroupNCCL):
...
def init_process_group(
    ...
    device_id: Optional[mindtorch.device] = None,
) -> None:
    """
    ...
    Args:
        ...
        device_id (mindtorch.device, optional): a single, specific device
...
def _get_split_source(pg):
    ...
    if pg.bound_device_id:
        ...
        try:
            split_from = pg._get_backend(mindtorch.device("cuda"))
        except RuntimeError:
...
def isend(
    ...
    if tensor.is_complex():
        tensor = mindtorch.view_as_real(tensor)
...
def _coalescing_manager(
    ...
    Args:
        group (`ProcessGroup`, optional): The process group to work on. If None,
            the default process group will be used.
        device (`mindtorch.device`, optional): Default is None, set to a device if
...
def batch_isend_irecv(p2p_op_list: List[P2POp]) -> List[Work]:
    ...
    Args:
        p2p_op_list: A list of point-to-point operations(type of each operator is
            ``mindtorch.distributed.P2POp``). The order of the isend/irecv in the list
    ...
    Examples:
        >>> # xdoctest: +SKIP("no rank")
        >>> send_tensor = mindtorch.arange(2, dtype=mindtorch.float32) + 2 * rank
        >>> recv_tensor = mindtorch.randn(2, dtype=mindtorch.float32)
    ...
    .. note:: Note that when this API is used with the NCCL PG backend, users must set
        the current GPU device with `mindtorch.cuda.set_device`, otherwise it will
...
def all_reduce(tensor, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    Args:
        ...
        op (optional): One of the values from
            ``mindtorch.distributed.ReduceOp``
    ...
    Examples:
        >>> # xdoctest: +SKIP("no rank")
        >>> # All tensors below are of mindtorch.int64 type.
        >>> # We have 2 process groups, 2 ranks.
        >>> device = mindtorch.device(f'cuda:{rank}')
        >>> tensor = mindtorch.arange(2, dtype=mindtorch.int64, device=device) + 1 + 2 * rank
        >>> tensor
        ...
        >>> # All tensors below are of mindtorch.cfloat type.
        >>> # We have 2 process groups, 2 ranks.
        >>> tensor = mindtorch.tensor([1+1j, 2+2j], dtype=mindtorch.cfloat, device=device) + 2 * rank * (1+1j)
        >>> tensor
...
@_exception_logger
@deprecated(
    "`mindtorch.distributed.all_reduce_coalesced` will be deprecated. If you must "
    "use it, please revisit our documentation later at "
    "https://pymindtorch.org/docs/main/distributed.html#collective-functions",
    category=FutureWarning,
)
def all_reduce_coalesced(tensors, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    For example, if the rank 0 node passes [mindtorch.rand(4), mindtorch.rand(2)] and the
    rank 1 node passes [mindtorch.rand(2), mindtorch.rand(2), mindtorch.rand(2)], the allreduce
    ...
    Args:
        ...
        op (Optional[ReduceOp]): One of the values from
            ``mindtorch.distributed.ReduceOp`` enum. Specifies an operation used for
    ...
    tensors = [t if not t.is_complex() else mindtorch.view_as_real(t) for t in tensors]
...
def _object_to_tensor(obj, device, group):
    ...
    byte_tensor = mindtorch.Tensor(mindtorch.Tensor.convert_bytes_to_tensor(byte_data, (len(byte_data),), mindtorch.int8))
    # Do not replace `mindtorch.ByteTensor` or `mindtorch.LongTensor` with mindtorch.tensor and specifying dtype.
    # Otherwise, it will casue 100X slowdown.
    # See: https://github.com/pytorch/pytorch/issues/65696
    local_size = mindtorch.Tensor([byte_tensor.numel()], dtype=mindtorch.int32)
...
def all_gather_object(object_list, obj, group=None):
    ...
    .. note:: For NCCL-based processed groups, internal tensor representations
        ...
        ``mindtorch.cuda.current_device()`` and it is the user's responsiblity to
        ensure that this is set so that each rank has an individual GPU, via
        ``mindtorch.cuda.set_device()``.
    ...
    object_sizes_tensor = mindtorch.zeros(
        group_size, dtype=mindtorch.int32
    )
    object_size_list = [
    ...
    if max_object_size - input_tensor.shape[0] > 0:
        input_tensor = mindtorch.concat([input_tensor, mindtorch.zeros(max_object_size - input_tensor.shape[0], dtype=input_tensor.dtype)])

    coalesced_output_tensor = mindtorch.empty(
        max_object_size * group_size, dtype=mindtorch.int8
    )
    ...
    for i, tensor in enumerate(output_tensors):
        tensor = tensor.type(mindtorch.int8)
...
def send_object_list(
    ...
    Args:
        ...
        group: (ProcessGroup, optional): The process group to work on. If None,
            the default process group will be used. Default is ``None``.
        device (``mindtorch.device``, optional): If not None, the objects are
    ...
    .. note:: For NCCL-based process groups, internal tensor representations
        of objects must be moved to the GPU device before communication takes
        place. In this case, the device used is given by
        ``mindtorch.cuda.current_device()`` and it is the user's responsibility to
        ensure that this is set so that each rank has an individual GPU, via
        ``mindtorch.cuda.set_device()``.
    ...
    object_sizes_tensor = mindtorch.cat(size_list)

    # Send object sizes
    send(object_sizes_tensor, group_dst=group_dst, group=group)

    # Concatenate and send serialized object tensors
    # Note: mindtorch.cat will do an extra memory copy to the current device, if the tensor_list
    # has only one element, we can skip the copy.
    if len(tensor_list) == 1:  # type: ignore[possibly-undefined]
        object_tensor = tensor_list[0]
    else:
        object_tensor = mindtorch.cat(tensor_list)
...
def recv_object_list(
    ...
    Args:
        ...
        group: (ProcessGroup, optional): The process group to work on. If None,
            the default process group will be used. Default is ``None``.
        device (``mindtorch.device``, optional): If not None, receives on this device.
            Default is ``None``.
        group_src (int, optional): Destination rank on ``group``.  Invalid to specify both ``src`` and ``group_src``.
    ...
    current_device = device or _get_object_coll_device(group)
    object_sizes_tensor = mindtorch.empty(
        len(object_list), dtype=mindtorch.long, device=current_device
    )
    ...
    object_tensor = mindtorch.empty(  # type: ignore[call-overload]
        mindtorch.sum(object_sizes_tensor).item(),  # type: ignore[arg-type]
        dtype=mindtorch.int8,
    ...
    for i, obj_size in enumerate(object_sizes_tensor):
        ...
        obj_view = obj_view.type(mindtorch.int8)
        offset += obj_size
...
def scatter_object_list(
    ...
    if my_global_rank == global_src:
        ...
        for i in range(len(tensor_list)):  # type: ignore[possibly-undefined]
            ...
            if max_tensor_size - tensor.shape[0] > 0:
                tensor = mindtorch.concat([tensor, mindtorch.zeros(max_tensor_size.item() - tensor.shape[0], dtype=tensor.dtype)])
            tensor_list[i] = tensor
    else:
        max_tensor_size = mindtorch.tensor([0], dtype=mindtorch.long)
    max_tensor_size = broadcast(max_tensor_size, src=global_src, group=group)

    # Scatter actual serialized objects
    output_tensor = mindtorch.empty(
        max_tensor_size.item(), dtype=mindtorch.int8
    )
    ...
    obj_tensor_size = mindtorch.tensor([0], dtype=mindtorch.int32)
...
def all_gather(tensor_list, tensor, group=None, async_op=False):
    ...
    Examples:
        >>> # xdoctest: +SKIP("need process group init")
        >>> # All tensors below are of mindtorch.int64 dtype.
        >>> # We have 2 process groups, 2 ranks.
        >>> device = mindtorch.device(f'cuda:{rank}')
        >>> tensor_list = [mindtorch.zeros(2, dtype=mindtorch.int64, device=device) for _ in range(2)]
        >>> tensor_list
        ...
        >>> # All tensors below are of mindtorch.cfloat dtype.
        >>> # We have 2 process groups, 2 ranks.
        >>> tensor_list = [mindtorch.zeros(2, dtype=mindtorch.cfloat, device=device) for _ in range(2)]
        >>> tensor_list
    ...
    tensor_list = [
        t if not t.is_complex() else mindtorch.view_as_real(t) for t in tensor_list
    ]
    tensor = tensor if not tensor.is_complex() else mindtorch.view_as_real(tensor)
...
def all_gather_into_tensor(output_tensor, input_tensor, group=None, async_op=False):
    ...
    Args:
        output_tensor (Tensor): Output tensor to accommodate tensor elements
            ...
            dimension; for definition of "concatenation", see ``mindtorch.cat()``;
            (ii) a stack of all the input tensors along the primary dimension;
            for definition of "stack", see ``mindtorch.stack()``.
    ...
    Examples:
        >>> # xdoctest: +SKIP("need process group init")
        >>> # All tensors below are of mindtorch.int64 dtype and on CUDA devices.
        >>> # We have two ranks.
        >>> device = mindtorch.device(f'cuda:{rank}')
        >>> tensor_in = mindtorch.arange(2, dtype=mindtorch.int64, device=device) + 1 + 2 * rank
        >>> tensor_in
        tensor([1, 2], device='cuda:0') # Rank 0
        tensor([3, 4], device='cuda:1') # Rank 1
        >>> # Output in concatenation form
        >>> tensor_out = mindtorch.zeros(world_size * 2, dtype=mindtorch.int64, device=device)
        >>> dist.all_gather_into_tensor(tensor_out, tensor_in)
        >>> tensor_out
        tensor([1, 2, 3, 4], device='cuda:0') # Rank 0
        tensor([1, 2, 3, 4], device='cuda:1') # Rank 1
        >>> # Output in stack form
        >>> tensor_out2 = mindtorch.zeros(world_size, 2, dtype=mindtorch.int64, device=device)
    ...
    output_tensor = (
        ...
        else mindtorch.view_as_real(output_tensor)
    )
    input_tensor = (
        input_tensor
        if not input_tensor.is_complex()
        else mindtorch.view_as_real(input_tensor)
    )
...
@_exception_logger
@deprecated(
    "`mindtorch.distributed._all_gather_base` is a private function and will be deprecated. "
    "Please use `mindtorch.distributed.all_gather_into_tensor` instead.",
    category=FutureWarning,
)
...
@_exception_logger
@deprecated(
    "`mindtorch.distributed.all_gather_coalesced` will be deprecated. If you must use it, "
...
def all_gather_coalesced(
    ...
    WARNING: at this time individual shape checking is not implemented across nodes.
    For example, if the rank 0 node passes [mindtorch.rand(4), mindtorch.rand(2)] and the
    rank 1 node passes [mindtorch.rand(2), mindtorch.rand(2), mindtorch.rand(2)], the
    ...
    output_tensor_lists = [
        [t if not t.is_complex() else mindtorch.view_as_real(t) for t in l]
        for l in output_tensor_lists
    ]
    input_tensor_list = [
        t if not t.is_complex() else mindtorch.view_as_real(t) for t in input_tensor_list
    ]
...
def gather(
    tensor: mindtorch.Tensor,
    gather_list: Optional[List[mindtorch.Tensor]] = None,
    dst: Optional[int] = None,
    group: Optional[ProcessGroup] = None,
    async_op: bool = False,
    group_dst: Optional[int] = None,
    ...
    Example::
        ...
        >>> tensor_size = 2
        >>> device = mindtorch.device(f'cuda:{rank}')
        >>> tensor = mindtorch.ones(tensor_size, device=device) + rank
        >>> if dist.get_rank() == 0:
        >>>     gather_list = [mindtorch.zeros_like(tensor, device=device) for i in range(2)]
        >>> else:
        >>>     gather_list = None
...
def scatter(
    tensor: mindtorch.Tensor,
    scatter_list: Optional[List[mindtorch.Tensor]] = None,
    src: Optional[int] = None,
    group: Optional[ProcessGroup] = None,
    async_op: bool = False,
    group_src: Optional[int] = None,
    ...
    Example::
        ...
        >>> tensor_size = 2
        >>> device = mindtorch.device(f'cuda:{rank}')
        >>> output_tensor = mindtorch.zeros(tensor_size, device=device)
        >>> if dist.get_rank() == 0:
        >>>     # Assumes world_size of 2.
        >>>     # Only tensors, all of which must be the same size.
        >>>     t_ones = mindtorch.ones(tensor_size, device=device)
        >>>     t_fives = mindtorch.ones(tensor_size, device=device) * 5
    ...
    if _rank_not_in_group(group):
        ...
        return
    scatter_list = [
        t if not t.is_complex() else mindtorch.view_as_real(t) for t in scatter_list
    ]
    tensor = tensor if not tensor.is_complex() else mindtorch.view_as_real(tensor)
...
def reduce_scatter_tensor(output, input, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    Args:
        ...
        input (Tensor): Input tensor to be reduced and scattered. Its size
            ...
            For definition of "concatenation", see ``mindtorch.cat()``.
            For definition of "stack", see ``mindtorch.stack()``.
        group (ProcessGroup, optional): The process group to work on. If None,
            the default process group will be used.
        async_op (bool, optional): Whether this op should be an async op.
    ...
    Examples:
        ...
        >>> # We have two ranks.
        >>> device = mindtorch.device(f'cuda:{rank}')
        >>> tensor_out = mindtorch.zeros(2, dtype=mindtorch.int64, device=device)
        >>> # Input in concatenation form
        >>> tensor_in = mindtorch.arange(world_size * 2, dtype=mindtorch.int64, device=device)
        >>> tensor_in
        ...
        tensor([4, 6], device='cuda:1') # Rank 1
        >>> # Input in stack form
        >>> tensor_in = mindtorch.reshape(tensor_in, (world_size, 2))
        >>> tensor_in
        tensor([[0, 1],
...
@deprecated(
    "`mindtorch.distributed._reduce_scatter_base` is a private function and will be deprecated. "
    "Please use `mindtorch.distributed.reduce_scatter_tensor` instead.",
    category=FutureWarning,
)
...
def all_to_all_single(
    ...
    Examples:
        ...
        >>> input = mindtorch.arange(4) + rank * 4
        >>> input
        tensor([0, 1, 2, 3])     # Rank 0
        tensor([4, 5, 6, 7])     # Rank 1
        tensor([8, 9, 10, 11])   # Rank 2
        tensor([12, 13, 14, 15]) # Rank 3
        >>> output = mindtorch.empty([4], dtype=mindtorch.int64)
        ...
        >>> # Another example with tensors of mindtorch.cfloat type.
        >>> input = mindtorch.tensor([1+1j, 2+2j, 3+3j, 4+4j], dtype=mindtorch.cfloat) + 4 * rank * (1+1j)
        >>> input
    ...
    if input.is_complex():
        input = mindtorch.view_as_real(input)
    if output.is_complex():
        output = mindtorch.view_as_real(output)
...
def all_to_all(output_tensor_list, input_tensor_list, group=None, async_op=False):
    ...
    Examples:
        ...
        >>> output = list(mindtorch.empty([4], dtype=mindtorch.int64).chunk(4))
    ...
    output_tensor_list = [
        t if not t.is_complex() else mindtorch.view_as_real(t) for t in output_tensor_list
    ]
...
def barrier(
    ...
    opts = BarrierOptions()
    # opts.device = mindtorch.device(_get_object_coll_device(group))
...
def monitored_barrier(
    ...
    """
    Synchronize processes similar to ``mindtorch.distributed.barrier``, but consider a configurable timeout.
...
def split_group(
    ...
    parent_backend = parent_pg._get_backend(mindtorch.device("cuda"))
    ...
    pg._register_backend(mindtorch.device("cuda"), backend_type, backend_class)
...
def new_group(
    ...
    .. warning::
        ...
        When using async variants of mindtorch.distributed communication APIs,
...
def _new_group_with_tag(
    ...
    ``mindtorch.distributed._functional_collectives`` for reference on how to use it.
    """
    global _world
...
def new_subgroups(
    ...
    Examples:
        ...
        >>> rank = dist.get_rank()
        >>> tensor = mindtorch.ones(1, device=rank) * rank
    ...
    if group_size is None:
        if not mindtorch.cuda.is_available():
            raise ValueError(
        ...
        group_size = mindtorch.cuda.device_count()
    if group_size <= 0:
