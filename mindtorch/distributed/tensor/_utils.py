...
from mindtorch._prims_common import ShapeType
...
def compute_global_tensor_info(
    tensor: mindtorch.Tensor, mesh: DeviceMesh, placements: Sequence[Placement]
...
def try_find_mesh_from_args(
    op_call: mindtorch._ops.OpOverload, args: Sequence[object]
) -> DeviceMesh:
    """
...
def normalize_to_torch_size(size) -> mindtorch.Size:  # type: ignore[no-untyped-def]
    """
    Unify variable types of size argument to mindtorch.Size
    Acceptable types include:
        int, Sequence[int], Tuple[int], Tuple[Sequence[int]],
        or mindtorch.Size
    """
    if isinstance(size, mindtorch.Size):
        return size
    ...
    return mindtorch.Size(torch_size)
