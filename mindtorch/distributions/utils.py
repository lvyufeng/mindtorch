...
def broadcast_all(*values):
    ...
    value is broadcasted based on the following rules:
      - `mindtorch.*Tensor` instances are broadcasted as per :ref:`_broadcasting-semantics`.
      - numbers.Number instances (scalars) are upcast to tensors having
        the same size and type as the first tensor passed to `values`.  If all the
        values are scalars, then they are upcasted to scalar Tensors.

    Args:
        values (list of `numbers.Number`, `mindtorch.*Tensor` or objects implementing __torch_function__)

    Raises:
        ValueError: if any of the values is not a `numbers.Number` instance,
            a `mindtorch.*Tensor` instance, or an instance implementing __torch_function__
    """
...
def tril_matrix_to_vec(mat: mindspore.Tensor, diag: int = 0) -> mindspore.Tensor:
    ...
    """
    n = mat.shape[-1]
    # if not mindtorch._C._get_tracing_state() and (diag < -n or diag >= n):
...
def vec_to_tril_matrix(vec: mindspore.Tensor, diag: int = 0) -> mindspore.Tensor:
    ...
    # if not mindtorch._C._get_tracing_state() and (round(n) - n > eps):
    #     raise ValueError(
