...
class _MultiDeviceReplicator:
    ...
    def __init__(self, master_tensor: mindtorch.Tensor) -> None:
        self.master = master_tensor
        self._per_device_tensors: Dict[mindtorch.device, mindtorch.Tensor] = {}

    def get(self, device: mindtorch.device) -> mindtorch.Tensor:
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def __init__(
        ...
        if self._device == "cuda":
            if enabled and mindtorch.cuda.amp.common.amp_definitely_not_available():
                warnings.warn(
                    "mindtorch.cuda.amp.GradScaler is enabled, but CUDA is not available.  Disabling."
                )
                self._enabled = False
        ...
        if self._enabled:
            ...
            self._scale: Optional[mindtorch.Tensor] = None
            self._growth_factor = growth_factor
            self._backoff_factor = backoff_factor
            self._growth_interval = growth_interval
            self._init_growth_tracker = 0
            # self._growth_tracker will be lazily initialized during the first call to scale()
            self._growth_tracker: Optional[mindtorch.Tensor] = None
    ...
    def _lazy_init_scale_growth_tracker(self, dev: mindtorch.device) -> None:
        assert self._growth_tracker is None, "_growth_tracker initialized before _scale"
        self._scale = mindtorch.full((), self._init_scale, dtype=mindtorch.float32, device=dev)
        self._growth_tracker = mindtorch.full(
            (), self._init_growth_tracker, dtype=mindtorch.int32, device=dev
        )

    @overload
    def scale(self, outputs: mindtorch.Tensor) -> mindtorch.Tensor:
        ...

    @overload
    def scale(self, outputs: List[mindtorch.Tensor]) -> List[mindtorch.Tensor]:
        ...

    @overload
    def scale(self, outputs: Tuple[mindtorch.Tensor, ...]) -> Tuple[mindtorch.Tensor, ...]:
        ...

    @overload
    def scale(self, outputs: Iterable[mindtorch.Tensor]) -> Iterable[mindtorch.Tensor]:
        ...

    def scale(
        self,
        outputs: Union[mindtorch.Tensor, Iterable[mindtorch.Tensor]],
    ) -> Union[mindtorch.Tensor, Iterable[mindtorch.Tensor]]:
        """
        ...
        def apply_scale(val: Union[mindtorch.Tensor, Iterable[mindtorch.Tensor]]):
            if isinstance(val, mindtorch.Tensor):
                if len(stash) == 0:
                    if self._scale is None:
    ...
    def _unscale_grads_(
        self,
        optimizer: mindtorch.optim.Optimizer,
        inv_scale: mindtorch.Tensor,
        found_inf: mindtorch.Tensor,
        allow_fp16: bool,
    ) -> Dict[mindtorch.device, mindtorch.Tensor]:
        ...
        per_device_and_dtype_grads: Dict[
            mindtorch.device, Dict[mindtorch.dtype, List[mindtorch.Tensor]]
        ] = defaultdict(lambda: defaultdict(list))
        with mindtorch.no_grad():
            for group in optimizer.param_groups:
                for param in group["params"]:
                    assert isinstance(param, mindtorch.Tensor)
                    if param.grad is None:
                        continue
                    if (not allow_fp16) and param.grad.dtype == mindtorch.float16:
                    ...
                    if param.grad.is_sparse:
                        ...
                        if param.grad.dtype is mindtorch.float16:
            ...
            for device, per_dtype_grads in per_device_and_dtype_grads.items():
                for grads in per_dtype_grads.values():
                    mindtorch._amp_foreach_non_finite_check_and_unscale_(
                        grads,
    ...
    def unscale_(self, optimizer: mindtorch.optim.Optimizer) -> None:
        """
        ...
        Simple example, using :meth:`unscale_` to enable clipping of unscaled gradients::
            ...
            mindtorch.nn.utils.clip_grad_norm_(model.parameters(), max_norm)
            scaler.step(optimizer)
            scaler.update()

        Args:
            optimizer (mindtorch.optim.Optimizer):  Optimizer that owns the gradients to be unscaled.
        ...
        found_inf = mindtorch.full((), 0.0, dtype=mindtorch.float32, device=self._scale.device)
    ...
    def step(
        self, optimizer: mindtorch.optim.Optimizer, *args: Any, **kwargs: Any
    ) -> Optional[float]:
        ...
        Args:
            optimizer (mindtorch.optim.Optimizer):  Optimizer that applies the gradients.
            args:  Any arguments.
    ...
    def update(self, new_scale: Optional[Union[float, mindtorch.Tensor]] = None) -> None:
        ...
        Args:
            new_scale (float or :class:`mindtorch.Tensor`, optional, default=None):  New scale factor.
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                ...
                reason = "new_scale should be a float or a 1-element mindtorch.cuda.FloatTensor or \
                    mindtorch.FloatTensor with requires_grad=False."
    ...
    def _get_scale_async(self) -> Optional[mindtorch.Tensor]:
        return self._scale
    ...
    def _check_inf_per_device(self, optimizer: mindtorch.optim.Optimizer) -> Dict[str, Any]:
        _scale, _ = self._check_scale_growth_tracker("_check_inf_per_device")

        dummy_inv_scale = mindtorch.full((), 1.0, dtype=mindtorch.float32, device=_scale.device)
        found_inf = mindtorch.full((), 0.0, dtype=mindtorch.float32, device=_scale.device)
    ...
    def _found_inf_per_device(self, optimizer: mindtorch.optim.Optimizer) -> Dict[str, Any]:
        return self._per_optimizer_states[id(optimizer)]["found_inf_per_device"]