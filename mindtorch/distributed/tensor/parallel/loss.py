...
import mindtorch
import mindtorch._prims_common as utils
...
from mindtorch.distributed.tensor import DTensor, Replicate, Shard
from mindtorch.distributed.tensor._dtensor_spec import DTensorSpec, TensorMeta
from mindtorch.distributed.tensor._ops._embedding_ops import _MaskPartial
from mindtorch.distributed.tensor._ops._math_ops import (
    _skip_dim,
    Reduction,
    replicate_reduction_dims,
)
from mindtorch.distributed.tensor.placement_types import Placement
...
def loss_parallel():
    ...
    Within this context manager, one can use :func:`~mindtorch.nn.functional.cross_entropy` or
    :class:`~mindtorch.nn.CrossEntropyLoss` as usual, with the following assumptions on the input parameters.
    ...
    Args:
        ...
        target (Union[:class:`mindtorch.Tensor`, :class:`DTensor`]):
        ...
        weight (Union[:class:`mindtorch.Tensor`, :class:`DTensor`], optional):
    ...
    Example:
        ...
        >>> from mindtorch.distributed.tensor.parallel import loss_parallel
        >>> from mindtorch.distributed.device_mesh import init_device_mesh
        >>> ...
        >>> device_mesh = init_device_mesh("cuda", (8,))
        >>> input = mindtorch.randn(4, 16, device="cuda", requires_grad=True)
        >>> dist_input = distribute_tensor(input, device_mesh, placements=[Shard(1)])
        >>> target = mindtorch.randint(16, (4,), device="cuda")
        >>> with loss_parallel():
...
def _cast_to_dtensor(
    ...
    elif isinstance(tensor, mindtorch.Tensor):
...
def _log_softmax_backward_handler(
    ...
    input_dtype = cast(mindtorch.dtype, args[3])
    return grad_output.to(input_dtype)


# NOTE: The implementation follows mindtorch._decomp.decomposition._nll_loss_forward,
...
def _nll_loss_forward(
    ...
    if weight is not None:
        ...
        x = x * local_w
    safe_target = mindtorch.where(target != ignore_index, target, 0)
    safe_target_ = safe_target.unsqueeze(channel_dim)

    # The following code block is a distributed version of
    # result = -mindtorch.gather(self, channel_dim, safe_target_).squeeze(channel_dim)
    ...
    result_partial = mindtorch.gather(x, channel_dim, safe_target_partial_)
    ...
    result = mindtorch.where(target != ignore_index, result, 0)
    ...
    if weight is not None:
        ...
        wsum = mindtorch.gather(w, channel_dim, safe_target_).squeeze(channel_dim)
        wsum = mindtorch.where(target != ignore_index, wsum, 0)
...
# from mindtorch._decomp.decomposition.
def _nll_loss_and_log_softmax_backward(
    ...
    safe_target = mindtorch.where(target != ignore_index, target, 0)
    grad_input = mindtorch.zeros_like(x)

    # The following code block is a distributed version of
    # grad_input = mindtorch.scatter(grad_input, channel_dim, safe_target, -1.0)
    partial_placement = _MaskPartial(offset_shape=input_shape, offset_dim=channel_dim)
    ...
    if weight is not None:
        ...
        w_target = mindtorch.gather(w, channel_dim, target)
        grad_output = grad_output * w_target

    grad_output = mindtorch.where(target != ignore_index, grad_output, 0)
    ...
    return (grad_input + mindtorch.exp(x)) * grad_output
