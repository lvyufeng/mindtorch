...
import mindtorch
from mindtorch import _linalg_utils as _utils, Tensor
from mindtorch.overrides import handle_torch_function, has_torch_function
...
def get_approximate_basis(
    ...
    R = mindtorch.randn(A.shape[-1], q, dtype=dtype, device=A.device)

    # The following code could be made faster using mindtorch.geqrf + mindtorch.ormqr
    # but geqrf is not differentiable

    X = matmul(A, R)
    if M is not None:
        X = X - matmul(M, R)
    Q = mindtorch.linalg.qr(X).Q
...
def svd_lowrank(
    ...
    .. note:: In general, use the full-rank SVD implementation
              :func:`mindtorch.linalg.svd` for dense matrices due to its 10x
              higher performance characteristics. The low-rank SVD
              will be useful for huge sparse matrices that
              :func:`mindtorch.linalg.svd` cannot handle.
...
def _svd_lowrank(
    ...
    if M is not None:
        B = B - matmul(Q.mH, M)
    U, S, Vh = mindtorch.linalg.svd(B, full_matrices=False)
    V = Vh.mH
    U = Q.matmul(U)
...
def pca_lowrank(
    ...
    if not mindtorch.jit.is_scripting():
        if type(A) is not mindtorch.Tensor and has_torch_function((A,)):
            return handle_torch_function(
    ...
    if _utils.is_sparse(A):
        ...
        c = mindtorch.sparse.sum(A, dim=(-2,)) / m
        # reshape c
        column_indices = c.indices()[0]
        indices = mindtorch.zeros(
            2,
            len(column_indices),
        ...
        C_t = mindtorch.sparse_coo_tensor(
            indices, c.values(), (n, 1), dtype=dtype, device=A.device
        )

        ones_m1_t = mindtorch.ones(A.shape[:-2] + (1, m), dtype=dtype, device=A.device)
        M = mindtorch.sparse.mm(C_t, ones_m1_t).mT
