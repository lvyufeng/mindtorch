...
from mindtorch.distributed.fsdp._common_utils import _FSDPState, _get_param_to_fqns
from mindtorch.distributed.fsdp._flat_param import FlatParamHandle
...
class _ExecOrderData:
    ...
    def _check_order(self, handle: FlatParamHandle, is_training: bool) -> None:
        ...
        if self.is_first_iter:
            ...
            tensor_kwargs: Dict[str, Union[mindtorch.dtype, mindtorch.device]] = {
                "dtype": mindtorch.int32,
                "device": device,
            }
            world_num_valid_indices = mindtorch.zeros(self.world_size, **tensor_kwargs)  # type: ignore[arg-type, call-overload]
            local_num_valid_indices = mindtorch.tensor([num_valid_indices], **tensor_kwargs)  # type: ignore[arg-type, call-overload]
            ...
            world_indices = mindtorch.zeros(  # type: ignore[call-overload]
                self.world_size * num_valid_indices, **tensor_kwargs
            )
            local_indices = mindtorch.tensor(optional_local_indices, **tensor_kwargs)  # type: ignore[arg-type]
            dist.all_gather_into_tensor(
