...
import mindtorch.distributed.tensor._dtensor_spec as dtensor_spec
# from mindtorch._C._distributed_c10d import _resolve_process_group
...
# if not mindtorch._running_with_deploy():

#     @mindtorch.library.register_fake("_dtensor::shard_dim_alltoall")
#     def _shard_dim_alltoall_meta(input, gather_dim, shard_dim, group_name):
#         group_size = _get_group_size_by_name(group_name)
#         stacked_list = [mindtorch.empty_like(input) for _ in range(group_size)]
#         group = _resolve_process_group(group_name)
#         group_rank = get_group_rank(group, get_rank())

#         return mindtorch.cat(stacked_list, dim=gather_dim).chunk(group_size, dim=shard_dim)[
#             group_rank
#         ]
...
def shard_dim_alltoall(input, gather_dim, shard_dim, mesh, mesh_dim):
    if mesh.device_type == "cpu":
        ...
        if isinstance(out, funcol.AsyncCollectiveTensor):
            ...
            out = out.wait()
        out = mindtorch.chunk(out, mesh.size(mesh_dim), dim=shard_dim)[
    ...
    return mindtorch.ops._dtensor.shard_dim_alltoall(
...
def mesh_scatter(
    ...
    scatter_list: List[mindtorch.Tensor],
    mesh: DeviceMesh,
    mesh_dim: int = 0,
    async_op: bool = False,
) -> Optional[Work]:
    ...
    Args:
        output (mindtorch.Tensor): the tensor to receive the scattered list.
        scatter_list (List[mindtorch.Tensor]): the tensor list to be scattered.
...
def mesh_broadcast(
    ...
    Args:
        tensor (mindtorch.Tensor): tensor to broadcast.
        mesh_dim (int, optional): indicate which mesh dimension we want
            to scatter on, we by default choose the first rank on the
            mesh dimension as source of truth.
...
def pad_tensor(tensor: mindtorch.Tensor, pad_dim: int, pad_size: int) -> mindtorch.Tensor:
    if pad_size == 0:
        return tensor
    pad = [0, 0] * (tensor.ndim - pad_dim)
    pad[-1] = pad_size
    return mindtorch.nn.functional.pad(tensor, pad)


def unpad_tensor(tensor: mindtorch.Tensor, pad_dim: int, pad_size: int) -> mindtorch.Tensor:
    if pad_size == 0:
        return tensor
...
def fill_empty_tensor_to_shards(
    shards: List[mindtorch.Tensor], shard_dim: int, num_empty_tensors: int
...
def check_tensor_meta(
    ...
    gathered_metadata = [None for _ in range(mindtorch.distributed.get_world_size())]
    mindtorch.distributed.all_gather_object(gathered_metadata, local_metadata)
