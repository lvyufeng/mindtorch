...
import mindtorch
from mindtorch import device, dtype, Tensor
...
def register_module_buffer_registration_hook(
    ...
    Returns:
        :class:`mindtorch.utils.hooks.RemovableHandle`:
...
def register_module_backward_hook(
    ...
    :func:`mindtorch.nn.modules.module.register_module_full_backward_hook`
...
def register_module_full_backward_pre_hook(
    ...
    registered by :meth:`mindtorch.nn.Module.register_full_backward_pre_hook`.
    Refer to its documentation for more details.

    Hooks registered using this function will be called before hooks registered
    using :meth:`mindtorch.nn.Module.register_full_backward_pre_hook`.
...
def register_module_full_backward_hook(
    ...
    Hooks registered using this function behave in the same way as those
    registered by :meth:`mindtorch.nn.Module.register_full_backward_hook`.
    Refer to its documentation for more details.

    Hooks registered using this function will be called before hooks registered
    using :meth:`mindtorch.nn.Module.register_full_backward_hook`.
...
class Module:
    ...
    def register_buffer(self, name: str, tensor: Optional[Tensor], persistent: bool = True) -> None:
        ...
        elif tensor is not None and not isinstance(tensor, mindtorch.Tensor):
    ...
    def _apply(self, fn, recurse=True):
        ...
        def compute_should_use_set_data(tensor, tensor_applied):
            if mindtorch._has_compatible_shallow_copy_type(tensor, tensor_applied):
                # If the new tensor has compatible tensor type as the existing tensor,
                # the current behavior is to change the tensor in-place using `.data =`,
                # and the future behavior is to overwrite the existing tensor. However,
                # changing the current behavior is a BC-breaking change, and we want it
                # to happen in future releases. So for now we introduce the
                # `mindtorch.__future__.get_overwrite_module_params_on_conversion()`
        ...
        for key, param in self._parameters.items():
            ...
            if p_should_use_swap_tensors:
                try:
                    if param_grad is not None:
                        ...
                        param.grad = None
                    param_applied = mindtorch.nn.Parameter(
                        param_applied, requires_grad=param.requires_grad
                    )
                    mindtorch.utils.swap_tensors(param, param_applied)
            ...
            if param_grad is not None:
                ...
                if p_should_use_swap_tensors:
                    ...
                    try:
                        mindtorch.utils.swap_tensors(param_grad, grad_applied)
                    except Exception as e:
                        raise RuntimeError(
    ...
    def _call_impl(self, *args, **kwargs):
        ...
        try:
            ...
            if bw_hook:
                if not isinstance(result, (mindtorch.Tensor, tuple)):
            ...
            if non_full_backward_hooks:
                var = result
                while not isinstance(var, mindtorch.Tensor):
                    if isinstance(var, dict):
                        var = next(v for v in var.values() if isinstance(v, mindtorch.Tensor))
                    else:
                        var = var[0]
                # grad_fn = var.grad_fn
        ...
        if isinstance(value, Parameter):
            ...
            if value is not None:
                raise TypeError(f"cannot assign '{type(value)}' as parameter '{name}' "
                                "(mindtorch.nn.Parameter or None expected)"
                                )
            self.register_parameter(name, value)
        else:
                ...
                if buffers is not None and name in buffers:
                    if value is not None and not isinstance(value, Tensor):
                        raise TypeError(f"cannot assign '{type(value)}' as buffer '{name}' "
                                        "(mindtorch.Tensor or None expected)"
                                        )
                    for hook in _global_buffer_registration_hooks.values():
                        output = hook(self, name, value)
                        if output is not None:
    ...
    def parameters(self, recurse: bool = True) -> Iterator[Parameter]:
        ...
        Example::
            ...
            <class 'mindtorch.Tensor'> (20L,)
            <class 'mindtorch.Tensor'> (20L, 1L, 5L, 5L)
    ...
    def buffers(self, recurse: bool = True) -> Iterator[Tensor]:
        ...
        Yields:
            mindtorch.Tensor: module buffer
    ...
    def named_buffers(self, prefix: str = '', recurse: bool = True, remove_duplicate: bool = True) -> Iterator[Tuple[str, Tensor]]:
        ...
        Yields:
            (str, mindtorch.Tensor): Tuple containing the name and buffer
    ...
    def to(self, *args, **kwargs):
        ...
        .. function:: to(memory_format=mindtorch.channels_last)
           :noindex:

        Its signature is similar to :meth:`mindtorch.Tensor.to`, but only accepts
        ...
        Args:
            ...
            dtype (:class:`mindtorch.dtype`): the desired floating point or complex dtype of
                the parameters and buffers in this module
            tensor (mindtorch.Tensor): Tensor whose dtype and device are the desired
                dtype and device for all parameters and buffers in this module
            memory_format (:class:`mindtorch.memory_format`): the desired memory
        ...
        Examples::
            ...
            tensor([[ 0.1913, -0.3420],
                    [-0.5113, -0.2325]])
            >>> linear.to(mindtorch.double)
            Linear(in_features=2, out_features=2, bias=True)
            >>> linear.weight
            Parameter containing:
            tensor([[ 0.1913, -0.3420],
                    [-0.5113, -0.2325]], dtype=mindtorch.float64)
            >>> # xdoctest: +REQUIRES(env:TORCH_DOCTEST_CUDA1)
            >>> gpu1 = mindtorch.device("cuda:1")
            >>> linear.to(gpu1, dtype=mindtorch.half, non_blocking=True)
            Linear(in_features=2, out_features=2, bias=True)
            >>> linear.weight
            Parameter containing:
            tensor([[ 0.1914, -0.3420],
                    [-0.5112, -0.2324]], dtype=mindtorch.float16, device='cuda:1')
            >>> cpu = mindtorch.device("cpu")
            >>> linear.to(cpu)
            Linear(in_features=2, out_features=2, bias=True)
            >>> linear.weight
            Parameter containing:
            tensor([[ 0.1914, -0.3420],
                    [-0.5112, -0.2324]], dtype=mindtorch.float16)

            >>> linear = nn.Linear(2, 2, bias=None).to(mindtorch.cdouble)
            >>> linear.weight
            Parameter containing:
            tensor([[ 0.3741+0.j,  0.2382+0.j],
                    [ 0.5593+0.j, -0.4443+0.j]], dtype=mindtorch.complex128)
            >>> linear(mindtorch.ones(3, 2, dtype=mindtorch.cdouble))
            tensor([[0.6122+0.j, 0.1150+0.j],
                    [0.6122+0.j, 0.1150+0.j],
                    [0.6122+0.j, 0.1150+0.j]], dtype=mindtorch.complex128)

        """
        device, dtype, non_blocking, convert_to_format = mindtorch._C._nn._parse_to(
            *args, **kwargs
        )
        ...
        def convert(t):
            try:
                ...
                if str(e) == "Cannot copy out of meta tensor; no data!":
                    raise NotImplementedError(
                        f"{e} Please use mindtorch.nn.Module.to_empty() instead of mindtorch.nn.Module.to() "
    ...
    def state_dict(self, *args, destination=None, prefix='', keep_vars=False):
        ...
        Args:
            ...
            keep_vars (bool, optional): by default the :class:`~mindtorch.Tensor` s
    ...
    def zero_grad(self, set_to_none: bool = True) -> None:
        ...
        See similar function under :class:`mindtorch.optim.Optimizer` for more context.

        Args:
            set_to_none (bool): instead of setting to zero, set the grads to None.
                See :meth:`mindtorch.optim.Optimizer.zero_grad` for details.
        """
