...
class _DynamicRNNBase(Module):
    ...
    def variable_recurrent(self, x, h, seq_length, w_ih, w_hh, b_ih, b_hh):
        ...
        seq_length = seq_length.to(mindtorch.float32)
        seq_length = ops.broadcast_to(seq_length, (hidden_size, -1))
        seq_length = seq_length.to(mindtorch.int32)
...
class _DynamicLSTMAscend(Module):
    ...
    def forward(self, x, h_0, seq_length, w_ih, w_hh, b_ih, b_hh):
        ...
        outputs, h, c, _, _, _, _, _ = self.lstm(x.to(mindtorch.float16), \
                                                 ops.transpose(weight, 1, 0).to(mindtorch.float16), \
                                                 bias.to(mindtorch.float16), None, \
                                                 h_0[0].unsqueeze(0).to(mindtorch.float16), \
                                                 h_0[1].unsqueeze(0).to(mindtorch.float16))
        if seq_length is not None:
...
class RNN(_RNNBase):
    ...
    Args:
        ...
        dtype (:class:`mindtorch.dtype`): Dtype of Parameters. Default: ``mstype.float32`` .

    Inputs:
        - **x** (Tensor) - Tensor of data type mindtorch.float32 or mindtorch.float16 and
          shape :math:`(seq\_len, batch\_size, input\_size)` or :math:`(batch\_size, seq\_len, input\_size)` .
        - **hx** (Tensor) - Tensor of data type mindtorch.float32 or mindtorch.float16 and
...
class LSTM(_RNNBase):
    ...
    Inputs:
        - **x** (Tensor) - Tensor of data type mindtorch.float32 or mindtorch.float16 and
          shape :math:`(seq\_len, batch\_size, input\_size)` or :math:`(batch\_size, seq\_len, input\_size)` .
        - **hx** (tuple) - A tuple of two Tensors (h_0, c_0) both of data type mindtorch.float32
          or mindtorch.float16 and shape :math:`(num\_directions * num\_layers, batch\_size, hidden\_size)` .
        - **seq_length** (Tensor) - The length of each sequence in an input batch.
          Tensor of shape :math:`(batch\_size)`. Default: ``None`` .
          This input indicates the real sequence length before padding to avoid padded elements
          have been used to compute hidden state and affect the final output. It is recommended to
