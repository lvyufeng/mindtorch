...
from mindtorch.distributed._state_dict_utils import _gather_state_dict
from mindtorch.distributed.distributed_c10d import _get_pg_default_device
...
from mindtorch.distributed.fsdp._debug_utils import SimpleProfiler
from mindtorch.distributed.fsdp._flat_param import FlatParameter, FlatParamHandle
from mindtorch.distributed.fsdp._fsdp_extensions import (
    _ext_chunk_dtensor,
    _ext_chunk_tensor,
)
...
from mindtorch.distributed.tensor import DTensor, Replicate
from mindtorch.utils._pytree import tree_map_only


if TYPE_CHECKING:
    from mindtorch.distributed._shard.sharded_tensor import ShardedTensor
...
class _ConsolidatedOptimState:
    ...
    Attributes:
        tensor_state (Dict[str, mindtorch.Tensor]): Mapping from positive-dimension
            tensor state name to the unsharded flat tensor representing the
            state.
        zero_dim_tensor_state (Dict[str, mindtorch.Tensor]): Mapping from zero-
            dimension tensor state name to its value.
        non_tensor_state (Dict[str, Any]): Mapping from non-tensor state
            name to its value.
    """

    tensor_state: Dict[str, mindtorch.Tensor] = field(default_factory=dict)
    zero_dim_tensor_state: Dict[str, mindtorch.Tensor] = field(default_factory=dict)
...
class _PosDimTensorInfo(NamedTuple):
    ...
    Attributes:
        shape (mindtorch.Size): Sharded tensor shape (which is equal to the
            unsharded tensor shape if the tensor is optimizer state for a
            non-FSDP parameter and is hence not sharded).
        dtype (mindtorch.dtype): Data type of the tensor.
    """

    shape: mindtorch.Size
        ...
        for optim_state in unflat_param_state:
            ...
            if cpu_offload:
                for key in list(optim_state.keys()):
                    ...
                    if not isinstance(state, mindtorch.Tensor):
                        continue
...
def _is_zero_dim_tensor(x: Any) -> bool:
    return mindtorch.is_tensor(x) and x.dim() == 0
...
def _communicate_optim_state(
    ...
    for state_name, value in sorted_items(flat_param_state):
        ...
        if mindtorch.is_tensor(value) and value.dim() > 0:
...
def _unflatten_communicated_optim_state(
    ...
    for _ in range(num_unflat_params):
        ...
        for state_name, flat_tensor in sorted_items(tensor_state):
            ...
            optim_state: Union[mindtorch.Tensor, ShardedTensor, DTensor] = next(views)
            if shard_state:
...
def _broadcast_state(
    ...
    if dist.get_rank(group) == 0:
        if not isinstance(state, mindtorch.Tensor) or state.dim() == 0:
            return state
        tensor = state.to(fsdp_state.compute_device)
    else:
        if isinstance(state, mindtorch.Tensor):
            assert state.dim() == 0, (
...
def _flatten_optim_state_dict(
    ...
    for param, fqns in param_to_fqns.items():
        ...
        if fqn in fqn_to_fsdp_param_info:
            ...
            # `mindtorch.optim.Optimizer` s unless the optimizer is KeyedOptimizer
            # or NamedOptimizer.
            if flat_state:
    ...
    for key in all_state_keys:
        ...
        if isinstance(user_state, mindtorch.Tensor) and rank0_only and use_orig_params:
...
def _flatten_optim_state(
    ...
    # Flatten the state
    flat_state: Dict[str, Optional[mindtorch.Tensor]] = {}
    for state_name in state_names:
        ...
        for v in non_none_state_values:
            are_pos_dim_tensors &= mindtorch.is_tensor(v) and v.dim() > 0
            are_zero_dim_tensors &= _is_zero_dim_tensor(v)
            are_non_tensors &= not mindtorch.is_tensor(v)
...
def _flatten_tensor_optim_state(
    state_name: str,
    pos_dim_tensors: List[mindtorch.Tensor],
    unflat_param_names: List[str],
    unflat_param_shapes: Sequence[mindtorch.Size],
    handle: FlatParamHandle,
) -> mindtorch.Tensor:
    """
    ...
    Args:
        ...
        pos_dim_tensors (List[mindtorch.Tensor]): Positive-dimension tensor
        ...
        unflat_param_shapes (List[mindtorch.Size]): Unflattened parameter shapes
            corresponding to the single flat parameter.
        handle (FlatParamHandle): The flat parameter's handle.

    Returns:
        mindtorch.Tensor: A flat tensor containing the optimizer state
    ...
    # parameter)
    cpu_device = mindtorch.device("cpu")
    tensors_to_flatten = [
        mindtorch.flatten(state_value.to(cpu_device))
        if state_value is not None
        else mindtorch.flatten(
            mindtorch.zeros(
                size=shape,
                dtype=dtype,
                device=cpu_device,
...
def _flatten_zero_dim_tensor_optim_state(
    state_name: str,
    zero_dim_tensors: List[mindtorch.Tensor],
    unflat_param_names: List[str],
) -> mindtorch.Tensor:
    """
    ...
    Args:
        state_name (str): Optimizer state name.
        zero_dim_tensors (List[mindtorch.Tensor]): Zero-dimension optimizer state
    ...
    Returns:
        mindtorch.Tensor: A zero-dimensional tensor giving the value of the state
    ...
    dtype = next(iter(dtypes))
    return mindtorch.tensor(value, dtype=dtype, device=mindtorch.device("cpu"))
...
def _get_param_id_to_param_from_optim_input(
    ...
    parameters or a list of parameter groups, :class:`mindtorch.optim.Optimizer`
    ...
    all_dicts = True
    for param in params:
        all_tensors &= isinstance(param, mindtorch.Tensor)
...
def _get_flat_param_to_fqn(model: mindtorch.nn.Module) -> Dict[FlatParameter, str]:
    """
...
def _check_missing_keys_on_rank(
    ...
    num_missing = mindtorch.tensor([len(missing_keys)], dtype=mindtorch.int32, device=device)
...
def _map_param_key_to_optim_keys(
    ...
    for param_key, param in param_key_to_param.items():
        ...
        # just like in normal `mindtorch.optim.Optimizer.state_dict()`
...
def _is_named_optimizer(optim_state_dict: Dict[str, Any]) -> bool:
    ...
    from a vanilla mindtorch.optim.Optimizer).
    """
...
class StateInfo:
    ...
    scalar_tensors: Dict[str, mindtorch.Tensor]
...
def _convert_all_state_info(
...
) -> Tuple[Optional[mindtorch.dtype], Dict[str, List[Optional[mindtorch.Tensor]]]]:
    """
    ...
    state_buffers: Dict[str, List[Optional[mindtorch.Tensor]]] = {}
    ...
    for fqn, gathered_state in output_states.items():
        ...
        dtype: Optional[mindtorch.dtype] = None
        ...
        for rank, object_state in enumerate(state_info):
            ...
            for name, scalar_tensor_value in object_state.scalar_tensors.items():
                ...
                assert curr_scalar_tensor_value is None or mindtorch.equal(
    ...
    for fqn, gathered_state in output_states.items():
        ...
        if isinstance(value, DTensor):
            ...
            if placement != Replicate():
                ...
                reshape_size = mindtorch.Size(reshape_size)
...
def _allgather_orig_param_states(
    ...
    empty_func = functools.partial(
        mindtorch.empty, dtype=dtype, device=fsdp_state.compute_device
    )
    ...
    for state_name, buffers in state_buffers.items():
        local_buffers: List[mindtorch.Tensor] = []
        ...
        for numel, is_padding in zip(
            ...
            if not is_padding:
                ...
                if buffers[param_idx] is not None:
                    local_buffers.append(cast(mindtorch.Tensor, buffers[param_idx]))
                param_idx += 1
        ...
        local_shard = mindtorch.cat(local_buffers)
            ...
            with SimpleProfiler.profile("none_fsdp_managed_copy"):
                ...
                if cpu_offload:
                    for state_name, value in sorted_items(
                        ...
                        if not mindtorch.is_tensor(value):
                            continue
...
def _optim_state_dict(
    ...
    following the convention of :meth:`mindtorch.optim.Optimizer.state_dict`,
    ...
    Returns:
        ...
        :meth:`mindtorch.optim.Optimizer.state_dict`. If ``rank0_only=False``,
