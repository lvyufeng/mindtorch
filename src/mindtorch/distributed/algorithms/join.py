...
class Joinable(ABC):
    ...
    @property
    @abstractmethod
    def join_device(self) -> mindtorch.device:
...
class Join:
    ...
    Example::
        ...
        >>> import mindtorch
        >>> import mindtorch.distributed as dist
        >>> import mindtorch.multiprocessing as mp
        >>> # xdoctest: +SKIP
        >>> import mindtorch.nn.parallel.DistributedDataParallel as DDP
        >>> import mindtorch.distributed.optim.ZeroRedundancyOptimizer as ZeRO
        >>> from mindtorch.distributed.algorithms.join import Join
        >>>
        >>> # On each spawned worker
        >>> def worker(rank):
        >>>     dist.init_process_group("nccl", rank=rank, world_size=2)
        >>>     model = DDP(mindtorch.nn.Linear(1, 1).to(rank), device_ids=[rank])
        >>>     optim = ZeRO(model.parameters(), mindtorch.optim.Adam, lr=0.01)
        >>>     # Rank 1 gets one more input than rank 0
        >>>     inputs = [mindtorch.tensor([1.]).to(rank) for _ in range(10 + rank)]
    ...
    def _get_num_nonjoined_procs(self):
        ...
        num_nonjoined_procs = mindtorch.zeros(1, device=self._device)
    ...
    def _notify_procs_to_terminate(self):
        ...
        """
        ones = mindtorch.ones(1, device=self._device)
    ...
    def notify_join_context(joinable: Joinable):
        ...
        ones = mindtorch.ones(1, device=device)
        ...
        if join_config.throw_on_early_termination:
            ...
            zeros = mindtorch.zeros(1, device=device)
