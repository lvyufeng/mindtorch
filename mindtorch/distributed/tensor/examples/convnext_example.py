...
class LayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-6, data_format=mindtorch.contiguous_format):
        super().__init__()
        self.weight = nn.Parameter(mindtorch.ones(normalized_shape))
        self.bias = nn.Parameter(mindtorch.zeros(normalized_shape))
        self.eps = eps
        self.data_format = data_format
        if self.data_format not in [mindtorch.contiguous_format]:
    ...
    def forward(self, x):
        ...
        x = (x - u) / mindtorch.sqrt(s + self.eps)
...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0.0, layer_scale_init_value=1e-6):
        ...
        self.norm = LayerNorm(dim, eps=1e-6, data_format=mindtorch.contiguous_format)
        ...
        self.gamma = (
            nn.Parameter(
                layer_scale_init_value * mindtorch.ones((dim, 1, 1)), requires_grad=True
            )
...
class DownSampling(nn.Module):
    def __init__(self, dim_in=3, dim_out=2, down_scale=4, norm_first=False):
        ...
        if norm_first:
            self.norm = LayerNorm(dim_in, eps=1e-6, data_format=mindtorch.contiguous_format)
            self.conv = nn.Conv2d(
            ...
            self.norm = LayerNorm(
                dim_out, eps=1e-6, data_format=mindtorch.contiguous_format
            )
...
class ConvNeXt(nn.Module):
    def __init__(
        ...
        dp_rates = [x.item() for x in mindtorch.linspace(0, drop_path_rate, sum(depths))]
        cur = 0
...
def _conv_fn(
    ...
    for name, param in module.named_parameters():
        dist_spec = [Replicate()]
        dist_param = mindtorch.nn.Parameter(
...
def train_convnext_example():
    ...
    criterion = mindtorch.nn.CrossEntropyLoss()
    optimizer = mindtorch.optim.Adam(model.parameters(), lr=1e-4, amsgrad=False)

    x = mindtorch.randn(*in_shape).to(device_type).requires_grad_()
    y_target = (
        mindtorch.empty(output_shape[0], dtype=mindtorch.long)
    ...
    max_reserved = mindtorch.cuda.max_memory_reserved()
    max_allocated = mindtorch.cuda.max_memory_allocated()
    print(
