...
def redistribute_local_tensor(
    ...
    *,
    ...
    """
    This redistribute the local tensor (mindtorch.Tensor) from the current DTensorSpec to
    ...
    has_symints = any(isinstance(s, mindtorch.SymInt) for s in current_spec.shape) or any(
        isinstance(s, mindtorch.SymInt) for s in target_spec.shape
    )
    if has_symints:
...
class Redistribute(mindtorch.autograd.Function):
    @staticmethod
