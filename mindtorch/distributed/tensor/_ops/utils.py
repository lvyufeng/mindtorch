...
from mindtorch.distributed.tensor._collective_utils import redistribute_cost
...
) -> Union[List[object], mindtorch.fx.immutable_collections.immutable_list]:  # type: ignore[valid-type]
    # During tracing, `aten.sum.dim_IntList` uses `immutable_list` for its args,
    # which is an object but treated as a list by the tracer. Therefore, keep
    # `immutable_list` intact here as well.
    if type(x) is list or isinstance(x, mindtorch.fx.immutable_collections.immutable_list):
        return x
    else:
        return [x]
...
def infer_broadcast_dims_map(
    common_shape: mindtorch.Size, input_shape: mindtorch.Size
) -> List[int]:
