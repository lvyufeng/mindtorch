...
def _is_zipfile(f) -> bool:
    ...
    # binary. Since we expect the files here to be generated by mindtorch.save or
    # mindtorch.jit.save, it's safe to only check the start bytes and avoid
...
def _check_seekable(f) -> bool:
    ...
    def raise_err_msg(patterns, e):
        for p in patterns:
            if p in str(e):
                msg = (str(e) + ". You can only mindtorch.load from a file that is seekable."
...
def load(f, map_location=None, pickle_module=pickle, *, weights_only=False, mmap=None, **pickle_load_args):
    ...
    Raises:
        ...
        RuntimeError: Raised if mmap argument is used without files saved with `mindtorch.save(_use_new_zipfile_serialization=True)`.
    """
    ...
    with _open_file_like(f, 'rb') as opened_file:
        if _is_zipfile(opened_file):
            ...
            # If we want to actually tail call to mindtorch.jit.load, we need to
        ...
        if mmap:
            raise RuntimeError("mmap can only be used with files saved with ",
                               "`mindtorch.save(_use_new_zipfile_serialization=True), "
                               "please mindtorch.save your checkpoint with this option in order to use mmap.")
...
def _legacy_load(f, pickle_module, **pickle_load_args):
    ...
    Raises:
        ...
        RuntimeError: Raised if there is an issue with the file-like object compatibility with mindtorch.load.
    """
    ...
    if f_should_read_directly and f.tell() == 0:
        ...
        try:
            ...
            if _is_zipfile(f):
                # .zip is used for mindtorch.jit.save and will throw an un-pickling error here
                raise RuntimeError(
                    f"{f.name} is a zip archive (did you mean to use mindtorch.jit.load()?)") from None
            # if not a tarfile, reset file offset and proceed
            f.seek(0)

    if not hasattr(f, 'readinto') and (3, 8, 0) <= sys.version_info < (3, 8, 2):
        raise RuntimeError(
            "mindtorch.load does not work with file-like objects that do not implement readinto on Python 3.8.0 and 3.8.1. "
    ...
    for k, v in result.items():
        ...
        if array.dtype == bfloat16 and not SUPPORT_BF16:
            array = array.astype(np.float16)
        new_result[k] = mindtorch.from_numpy(array)
...
def _load(zip_file, pickle_module, overall_storage=None, pickle_file='data.pkl', **pickle_load_args):
    ...
    if not zip_file.has_record(byteordername) and \
        ...
        warnings.warn("The default load endianness for checkpoints without a byteorder mark "
                      ...
                      "mindtorch.serialization.set_default_load_endianness to set "
    ...
    load_module_mapping: Dict[str, str] = {
        ...
        'mindtorch.tensor': 'mindtorch._tensor'
    }
...
def convert_torch_to_mindspore(pth_file):
    ...
    try:
        import mindtorch # pylint: disable=import-error
    except Exception as exc:
        raise ImportError("'import mindtorch' failed, please install torch by "
                        "`pip install torch` or instructions from 'https://pymindtorch.org'") \
                        from exc
    if pth_file.endswith(".safetensors"):
        ...
        state_dict = mindtorch.load(pth_file, map_location='cpu')
    ...
    for key, value in state_dict.items():
        if value.dtype == mindtorch.bfloat16:
            data = mindtorch.from_numpy(value.to(mindtorch.float).numpy().astype(np.float16))
            if not has_bf16:
                has_bf16 = True
        else:
            data = mindtorch.from_numpy(value.numpy())
    ...
    try:
        ...
        for k, v in safeview:
            ...
            if (not SUPPORT_BF16 and dtype != bfloat16) or SUPPORT_BF16:
                result[k] = mindtorch.from_numpy(arr)
            else:
                result[k] = mindtorch.from_numpy(arr.astype(np.float16))
        return result
...
def safe_load_file(filename):
    ...
    def convert(info: dict[str, Any]):
        ...
        try:
            ...
            out = mindtorch.from_numpy(array)
        return out
    ...
    try:
        ...
        for element_id, element in enumerate(checkpoint_list.value):
            ...
            if data_type == "BFloat16":
                ...
                parameter = mindtorch.from_numpy(param_data)
