...
import mindtorch.distributed._shard.sharding_spec as shard_spec
import mindtorch.distributed.distributed_c10d as c10d
from mindtorch.distributed._shard.sharded_tensor import (
    Shard,
    ShardedTensor,
    ShardedTensorMetadata,
    TensorProperties,
)
from mindtorch.distributed._shard.sharding_spec import ShardMetadata
from mindtorch.distributed._shard.sharding_spec.chunk_sharding_spec import ChunkShardingSpec
from mindtorch.distributed.device_mesh import _mesh_resources
from mindtorch.distributed.fsdp._common_utils import _set_fsdp_flattened
from mindtorch.distributed.fsdp._fsdp_extensions import FSDPExtensions
from mindtorch.distributed.fsdp._shard_utils import _create_chunk_sharded_tensor
from mindtorch.distributed.remote_device import _remote_device
from mindtorch.distributed.tensor import DeviceMesh, DTensor, Replicate, Shard as DShard
from mindtorch.distributed.tensor.parallel._data_parallel_utils import (
    _flatten_tensor,
    _unflatten_tensor,
)
...
def _get_box(tensor: DTensor) -> Tuple[mindtorch.Size, mindtorch.Size]:
    ...
    return (mindtorch.Size(offsets), tensor._local_tensor.size())


def _get_box_for(tensor: DTensor, idx: int) -> Tuple[mindtorch.Size, mindtorch.Size]:
    offsets, size = _get_box(tensor)
    return (mindtorch.Size([val * idx for val in offsets]), size)


def _get_local_box(tensor: DTensor) -> Tuple[mindtorch.Size, mindtorch.Size]:
    device_mesh = tensor.device_mesh
...
def _rewrite_spec_if_needed(
    spec: shard_spec.ShardingSpec, tensor: mindtorch.Tensor, rank: int
...
def _chunk_dtensor(
    ...
    if isinstance(tensor, mindtorch.Tensor) and not isinstance(tensor, DTensor):
...
class DTensorExtensions(FSDPExtensions):
    ...
    def __init__(self, device_handle) -> None:
        ...
        self.post_unflatten_transform = mindtorch._dynamo.disable(self.post_unflatten_transform)  # type: ignore[method-assign]
    ...
    def post_unflatten_transform(
        self, tensor: mindtorch.Tensor, param_extension: Any
    ) -> mindtorch.Tensor:
