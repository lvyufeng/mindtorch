...
TensorListList: TypeAlias = List[List[mindtorch.Tensor]]
...
_foreach_supported_types = [mindtorch.Tensor, Parameter]
...
def _get_value(x):
    ...
    return x.item() if isinstance(x, mindtorch.Tensor) else x
...
def _get_scalar_dtype(is_fused=None):
    ...
    return (
        mindtorch.float64 if get_default_dtype() == mindtorch.float64 else mindtorch.float32
    )
...
ParamsT: TypeAlias = Union[Iterable[mindtorch.Tensor], Iterable[Dict[str, Any]]]
...
class Optimizer:
    ...
    Args:
        params (iterable): an iterable of :class:`mindtorch.Tensor` s or
    ...
    def _optimizer_step_code(self) -> None:
        """Entry point for `mindtorch.profile.profiler`.
    ...
    def register_state_dict_pre_hook(
        ...
        r"""Register a state dict pre-hook which will be called before :meth:`~mindtorch.optim.Optimizer.state_dict` is called.
        ...
        Returns:
            :class:`mindtorch.utils.hooks.RemoveableHandle`:
                a handle that can be used to remove the added hook by calling
                ``handle.remove()``
        """
    ...
    def register_state_dict_post_hook(
        ...
        r"""Register a state dict post-hook which will be called after :meth:`~mindtorch.optim.Optimizer.state_dict` is called.
    ...
    def state_dict(self) -> StateDict:
        ...
        * ``param_groups``: a List containing all parameter groups where each
        ...
        packed_state = {
            (param_mappings[id(k)] if isinstance(k, mindtorch.Tensor) else k): v
    ...
    def _process_value_according_to_param_policy(
        ...
        if key == "step":
            if capturable or fused:
                return value.to(dtype=mindtorch.float32)
            else:
                return value
        else:
    ...
    def register_load_state_dict_pre_hook(
        ...
        :meth:`~mindtorch.optim.Optimizer.load_state_dict` is called. It should have the
        following signature::
    ...
    def load_state_dict(self, state_dict: StateDict) -> None:
        ...
        state: DefaultDict[mindtorch.Tensor, Dict[Any, Any]] = defaultdict(dict)
    ...
    def add_param_group(self, param_group: Dict[str, Any]) -> None:
        ...
        param_set: Set[mindtorch.Tensor] = set()
    ...
    def zero_grad(self, set_to_none: bool = True) -> None:
        r"""Reset the gradients of all optimized :class:`mindtorch.Tensor` s.
        ...
        Args:
            set_to_none (bool): instead of setting to zero, set the grads to None.
                ...
                3. ``mindtorch.optim`` optimizers have a different behavior if the gradient is 0 or None
