...
import mindtorch
# import mindtorch.distributed.tensor._dispatch as op_dispatch
import mindtorch.distributed.tensor._random as random
import mindtorch.nn as nn
from mindtorch.distributed.device_mesh import _mesh_resources, DeviceMesh
from mindtorch.distributed.tensor._collective_utils import check_tensor_meta, mesh_broadcast
from mindtorch.distributed.tensor._dtensor_spec import DTensorSpec, TensorMeta
from mindtorch.distributed.tensor._random import (
    is_rng_supported_mesh,
    OffsetBasedRNGTracker,
)
from mindtorch.distributed.tensor._redistribute import (
    Redistribute,
    redistribute_local_tensor,
)
from mindtorch.distributed.tensor._utils import (
...
# NOTE [Autograd interaction between mindtorch.Tensor]
#
# The autograd functions defined below are being used by the public
# facing APIs (i.e. from_local, to_local) to ensure DTensor to work
# together with mindtorch.Tensor within the autograd engine. This
...
class _ToTorchTensor(mindtorch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output: mindtorch.Tensor):  # type: ignore[override]
...
class _FromTorchTensor(mindtorch.autograd.Function):
    @staticmethod
    def forward(  # type: ignore[override]
        ctx,  # pyre-ignore[2]: Parameter must be annotated.
        input: mindtorch.Tensor,
        device_mesh: DeviceMesh,
        placements: Tuple[Placement, ...],
        run_check: bool,
        shape: Optional[mindtorch.Size] = None,
        ...
        if shape and stride:
            ...
            tensor_shape, tensor_stride = mindtorch.Size(global_shape), tuple(global_stride)
        else:
            raise RuntimeError(
...
class DTensor(mindtorch.Tensor):
    """
    ``DTensor`` (Distributed Tensor) is a subclass of ``mindtorch.Tensor`` that provides single-device like
    abstraction to program with multi-device ``mindtorch.Tensor``. It describes the distributed tensor sharding
    ...
    * :class:`Partial`: Tensor is pending reduction on the devices of the ``DeviceMesh`` dimension
    ...
    _local_tensor: mindtorch.Tensor
    _spec: DTensorSpec
    ...
    @staticmethod
    @mindtorch._disable_dynamo
    def __new__(
        cls,
        local_tensor: mindtorch.Tensor,
        spec: DTensorSpec,
        *,
        requires_grad: bool,
    ) -> "DTensor":
        ...
        if local_tensor.requires_grad and not requires_grad:
            warnings.warn(
                "To construct DTensor from mindtorch.Tensor, it's recommended to "
        ...
        r = mindtorch.Tensor._make_wrapper_subclass(  # type: ignore[attr-defined]
            cls,
    ...
    def from_local(
        ...
        *,
        ...
        """
        Create a :class:`DTensor` from a local mindtorch.Tensor on each rank
        according to the ``device_mesh`` and ``placements`` specified.

        Args:
            local_tensor (mindtorch.Tensor): local mindtorch.Tensor on each rank.
            device_mesh (:class:`DeviceMesh`, optional): DeviceMesh to place the
                tensor, if not specified, must be called under a DeviceMesh
                context manager, default: None
            placements (List[:class:`Placement`], optional): the placements that
                describes how to place the local mindtorch.Tensor on DeviceMesh, must
        ...
        Keyword args:
            ...
            shape (mindtorch.Size, optional): A List of int which specifies the size of
    ...
    def to_local(
        ...
        Returns:
            A :class:`mindtorch.Tensor` or ``AsyncCollectiveTensor`` object. it represents the
    ...
    def redistribute(
        ...
        *,
        ...
        3. ``Replicate()`` -> ``Shard(dim)``: local chunking (i.e. ``mindtorch.chunk``)
    ...
    def full_tensor(
        ...
        Keyword args:
            grad_placements (List[:class:`Placement`], optional): the placements describes
                ...
                function.
                `full_tensor` converts DTensor to a full mindtorch.Tensor and the returned mindtorch.tensor
        ...
        Returns:
            A :class:`mindtorch.Tensor` object that represents the full tensor of this DTensor.
    ...
    def __create_write_items__(self, fqn: str, object: Any):
        from mindtorch.distributed.checkpoint.planner_helpers import (
        ...
        elif isinstance(self._local_tensor, mindtorch.Tensor):
...
def distribute_tensor(
    ...
    """
    Distribute a leaf ``mindtorch.Tensor`` (i.e. nn.Parameter/buffers) to the ``device_mesh`` according
    ...
    Args:
        tensor (mindtorch.Tensor): mindtorch.Tensor to be distributed. Note that if you
            want to shard a tensor on a dimension that is not evenly divisible by
            the number of devices in that mesh dimension, we use ``mindtorch.chunk``
    ...
    mindtorch._C._log_api_usage_once("mindtorch.dtensor.distribute_tensor")
    ...
    if device_type == "xla":
        try:
            ...
            from mindtorch_xla.distributed.spmd import (  # type:ignore[import]
...
def distribute_module(
    ...
    :class:`DTensor`, convert the output back to ``mindtorch.Tensor``)
    ...
    Args:
        ...
        output_fn (Callable): specify the output distribution, i.e. could control how the
            output is sharded, or convert it back to mindtorch.Tensor. ``output_fn`` will be
    ...
    mindtorch._C._log_api_usage_once("mindtorch.dtensor.distribute_module")
...
def _dtensor_init_helper(  # type: ignore[no-untyped-def]
    ...
    **kwargs,
    ...
    # from mindtorch.distributed._tensor.placement_types import DTensorSpec, TensorMeta
    ...
    assert kwargs["layout"] == mindtorch.strided, "layout value not supported!"
    torch_stride = mindtorch._prims_common.make_contiguous_strides_for(size)
    ...
    if init_op == mindtorch.full:
        fill_value = kwargs.pop("fill_value", 0)
        local_tensor = init_op(local_shape, fill_value, **kwargs)
    elif init_op == mindtorch.rand or init_op == mindtorch.randn:
        # this tensor meta is not used except `shape`
        dtype = kwargs.get("dtype", mindtorch.get_default_dtype())
...
def ones(  # type: ignore[no-untyped-def]
    *size,
    ...
    layout: mindtorch.layout = mindtorch.strided,
    ...
    Keyword args:
        dtype (:class:`mindtorch.dtype`, optional): the desired data type of returned :class:`DTensor`.
            Default: if ``None``, uses a global default (see :func:`mindtorch.set_default_dtype`).
        layout (:class:`mindtorch.layout`, optional): the desired layout of returned DTensor.
            Default: ``mindtorch.strided``.
...
def empty(  # type: ignore[no-untyped-def]
    *size,
    ...
    Keyword args:
        dtype (:class:`mindtorch.dtype`, optional): the desired data type of returned :class:`DTensor`.
            Default: if ``None``, uses a global default (see :func:`mindtorch.set_default_dtype`).\
        layout (:class:`mindtorch.layout`, optional): the desired layout of returned :class:`DTensor`.
            Default: ``mindtorch.strided``.
        requires_grad (bool, optional): If autograd should record operations on the
            returned :class:`DTensor`. Default: ``False``.
        device_mesh: :class:`DeviceMesh` type, contains the mesh info of ranks
