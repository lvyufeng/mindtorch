...
from mindtorch.amp.grad_scaler import _MultiDeviceReplicator, GradScaler, OptState
from mindtorch.distributed.distributed_c10d import ProcessGroup
...
def _is_supported_device(tensor: mindtorch.Tensor) -> bool:
...
class ShardedGradScaler(GradScaler):
    ...
    * Sync inf/nan for scaled gradient tensors on any mindtorch.device (where tensors are placed) across
    nodes
    ...
    Args:
        ...
        enabled (bool, optional):  If ``False``, disables gradient scaling. :meth:`step` simply
            ...
            Default: ``True``
        process_group (ProcessGroup, optional, default=mindtorch.distributed.group.WORLD):
    ...
    def scale(
        self, outputs: Union[mindtorch.Tensor, Iterable[mindtorch.Tensor]]
                ...
                for param in group["params"]:
                    ...
                    if param.grad.is_sparse:
                        ...
                        if param.grad.dtype is mindtorch.float16:
                            # coalesce is not supported in mindtorch.float16
                            param_grad_fp32 = param.grad.type(mindtorch.float32).coalesce()
                            param.grad = param_grad_fp32.type(mindtorch.float16)
    ...
    def unscale_(self, optimizer: mindtorch.optim.Optimizer) -> None:
        ...
        found_inf = mindtorch.full(
            (1,), 0.0, dtype=mindtorch.float32, device=self._scale.device
        )
        ...
        if found_inf_on_cpus:
            mindtorch._foreach_copy_(found_inf_on_cpus, found_inf_on_devices)

    def _amp_update_scale_cpu_(self, found_inf: mindtorch.Tensor) -> None:
        """
