...
from funcmindtorch.compile import make_boxed_func
from mindtorch._funcmindtorch.compilers import aot_module
from mindtorch._inductor.decomposition import select_decomp_table
from mindtorch.distributed.tensor import DTensor


inductor_decomps = select_decomp_table()

graphs: List[mindtorch.fx.GraphModule] = []
...
def print_op_coverage_summary(model: nn.Module, args, kwargs, *, output_csv=False):
    ...
    for node in fwd_graph.graph.nodes:
        if node.op == "call_function" and isinstance(
            node.target, mindtorch._ops.OpOverload
        ):
