...
class TracingConfig:
    ...
    Args:
        tracer (mindtorch.fx.Tracer): An instance of :class:`mindtorch.fx.Tracer` to
            use for symbolic tracing. The default value is the native
            :class:`mindtorch.fx.Tracer` constructed with default arguments.
        ...
        concrete_args (Optional[Dict[str, Any]]): Concrete arguments that
            should not be treated as ``mindtorch.fx.Proxy`` when tracing the
            ...
            in :meth:`~mindtorch.fx.Tracer.trace`.
    """

    tracer: mindtorch.fx.Tracer = field(default_factory=mindtorch.fx.Tracer)
...
class _ExecOrderTracer:
    ...
    @contextmanager
    def patch_tracer(self, tracer: mindtorch.fx.Tracer, root_module: nn.Module):
    ...
    def _patched_create_proxy(
        ...
        kind: str,
        target: mindtorch.fx.node.Target,
        args: Tuple[Any, ...],
        kwargs: Dict[str, Any],
        name: Optional[str] = None,
        type_expr: Optional[Any] = None,
        proxy_factory_fn: Optional[Callable[[mindtorch.fx.Node], mindtorch.fx.Proxy]] = None,
    ) -> mindtorch.fx.Proxy:
        """
        ...
        Args:
            ...
            kind (str): Kind of the target method ('call_function',
                ...
                'output'). See :class:`mindtorch.fx.Graph` for details. This is
                passed to ``create_proxy``.
            target (mindtorch.fx.node.Target): Contains the string name of the
            ...
            type_expr (Optional[Any]): An optional type annotation representing
                ...
                to ``create_proxy``.
            proxy_factory_fn (Callable[[mindtorch.fx.Node], mindtorch.fx.Proxy]):
                An alternative proxy constructor used in ``create_proxy``. This
                is passed to ``create_proxy``.

        Returns:
            mindtorch.fx.Proxy: Created ``Node`` wrapped in a ``Proxy`` object.
        """
        proxy = create_proxy(
        ...
        if kind in ("call_function", "call_method"):
            if args is not None:
                ...
                for arg in args:
                    if (
                        isinstance(arg, mindtorch.fx.Proxy)
