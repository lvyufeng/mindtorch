...
from mindtorch.distributed.tensor._collective_utils import (
...
class Shard(Placement):
    ...
    ``Shard(dim)`` placement follows the ``mindtorch.chunk(dim)`` semantic, where the
    ...
    def _split_tensor(
        ...
        *,
    ...
    ) -> Tuple[List[mindtorch.Tensor], List[int]]:
        """
        This function uses mindtorch.chunk to split a tensor into num_chunks shards along
        ...
        tensor_list = list(mindtorch.chunk(tensor, num_chunks, dim=self.dim))
        ...
        # compute the chunk size inline with ``mindtorch.chunk`` to calculate padding
    ...
    def _local_shard_size_on_dim(
        ...
        """
        # Compute the chunk size inline with ``mindtorch.chunk``
    ...
    def _shard_tensor(
        self, tensor: mindtorch.Tensor, mesh: DeviceMesh, mesh_dim: int
    ) -> mindtorch.Tensor:
        """
        ...
        output = mindtorch.empty_like(scatter_list[mesh_dim_local_rank])
    ...
    def _reduce_shard_tensor(
        ...
        if is_padded:
            ...
            tensor = mindtorch.cat(scattered_list, dim=self.dim)
...
class _StridedShard(Shard):
    ...
    def _split_tensor(
        ...
        *,
        ...
        total_split_tensor_list = list(mindtorch.chunk(tensor, total_split, dim=self.dim))
        tensor_list = [
            mindtorch.cat(
                [
    ...
    def _to_replicate_tensor(
        ...
        tensor_shard_list = mindtorch.chunk(result, total_split, dim=self.dim)
        # rearrange the order
        ...
        return mindtorch.cat(new_tensor_shard_list, dim=self.dim).contiguous()
...
class Replicate(Placement):
    ...
    def _replicate_tensor(
        ...
        """
        Replicate (broadcast) a mindtorch.Tensor on a mesh dimension (use
