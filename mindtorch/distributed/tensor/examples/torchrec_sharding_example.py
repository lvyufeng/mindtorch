...
import mindtorch
from mindtorch.distributed.checkpoint.metadata import (
...
def get_device_type():
    return (
        "cuda"
        if mindtorch.cuda.is_available() and mindtorch.cuda.device_count() >= 4
        else "cpu"
    )
...
# this mindtorch.Tensor subclass is a wrapper around all local shards associated
# with a single sharded embedding table.
class LocalShardsWrapper(mindtorch.Tensor):
    local_shards: List[mindtorch.Tensor]
    storage_meta: TensorStorageMetadata

    @staticmethod
    def __new__(
        cls, local_shards: List[mindtorch.Tensor], offsets: List[mindtorch.Size]
    ) -> "LocalShardsWrapper":
        ...
        wrapper_shape = mindtorch.Size(cat_tensor_shape)
        chunks_meta = [
    ...
    @property
    def shards(self) -> List[mindtorch.Tensor]:
        return self.local_shards

    @shards.setter
    def shards(self, local_shards: List[mindtorch.Tensor]):
        self.local_shards = local_shards

    @cached_property
    def shard_sizes(self) -> List[mindtorch.Size]:
        return [chunk.sizes for chunk in self.storage_meta.chunks]

    @cached_property
    def shard_offsets(self) -> List[mindtorch.Size]:
...
def run_torchrec_row_wise_even_sharding_example(rank, world_size):
    ...
    device = mindtorch.device(device_type)
    ...
    embedding_dim = 16
    # tensor shape
    local_shard_shape = mindtorch.Size(
        [num_embeddings // world_size, embedding_dim]  # (local_rows, local_cols)
    )
    # tensor offset
    local_shard_offset = mindtorch.Size((rank * 2, embedding_dim))
    # tensor
    local_tensor = mindtorch.randn(local_shard_shape, device=device)
    ...
    #   _initialize_torch_state() and mindtorch.Tensor params are transformed
    ...
    assert mindtorch.equal(shard_tensor, local_tensor)
...
def run_torchrec_row_wise_uneven_sharding_example(rank, world_size):
    ...
    num_embeddings = 8
    embedding_dim = 16
    emb_table_shape = mindtorch.Size([num_embeddings, embedding_dim])
    # tensor shape
    local_shard_shape = (
        mindtorch.Size([1, embedding_dim])
        if rank % 2 == 0
        else mindtorch.Size([3, embedding_dim])
    )
    # tensor offset
    local_shard_offset = mindtorch.Size((rank // 2 * 4 + rank % 2 * 1, embedding_dim))
    # tensor
    local_tensor = mindtorch.randn(local_shard_shape, device=device)
    # local shards
    # row-wise sharding: one shard per rank
    ...
    dtensor = DTensor.from_local(
        local_shards_wrapper,  # a mindtorch.Tensor subclass
...
def run_torchrec_table_wise_sharding_example(rank, world_size):
    ...
    for i in range(world_size):
        # tensor
        local_tensor = (
            mindtorch.randn(*emb_table_shape, device=device)
            if rank == i
            else mindtorch.empty(0, device=device)
        )
        table_to_local_tensor[i] = local_tensor
        # tensor offset
        local_shard_offset = mindtorch.Size((0, 0))
    ...
    for table_id, local_shards in table_to_shards.items():
        ...
        device_submesh = DeviceMesh(
            ...
            mesh=mindtorch.tensor(
                [table_id], dtype=mindtorch.int64
    ...
    ###########################################################################
    # example 2: transform DTensor into mindtorch.Tensor
    for table_id, local_tensor in table_to_local_tensor.items():
        # important: note that DTensor.to_local() always returns an empty mindtorch.Tensor
        # no matter what was passed to DTensor._local_tensor.
        dtensor_local_shards = table_to_dtensor[table_id].to_local()
        if rank == table_id:
            assert isinstance(dtensor_local_shards, LocalShardsWrapper)
            shard_tensor = dtensor_local_shards.shards[0]
            assert mindtorch.equal(shard_tensor, local_tensor)  # unwrap tensor
            assert (
                dtensor_local_shards.shard_sizes[0] == emb_table_shape
            )  # unwrap shape
            assert dtensor_local_shards.shard_offsets[0] == mindtorch.Size(
                (0, 0)
            )  # unwrap offset
        else:
