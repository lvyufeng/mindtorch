...
def sumproduct_pair(left_, right_, sum_dims_, keep_dim_):
    ...
    if len(sum_dims_) == 0:
        return mindtorch.mul(left_, right_)
    ...
    for i in range(dim):
        ...
        if sum_dims[i]:
            if sl and sr:
                ...
                left = mindtorch.sum(left, i, keepdim=True)
            elif sr:
                right = mindtorch.sum(right, i, keepdim=True)
        elif sl and sr:
            assert (
                left.shape[i] == right.shape[i]
            ), "non-broadcast dimensions must match"
    ...
    left = mindtorch.permute(left, tuple(lpermutation)).reshape(lro_size, lo_size, sum_size)
    right = mindtorch.permute(right, tuple(rpermutation)).view(lro_size, sum_size, ro_size)

    result = mindtorch.bmm(left, right)
...
def einsum(equation, *operands):
    ...
    for i, operand in enumerate(operands):
        ...
        for label in labels:
            if label == ELLIPSIS:
                ...
                for k in range(num_missing_dim):
                    operand = mindtorch.unsqueeze(operand, j)
                for k in range(ell_num_dim):
                    perm_shape[ell_index + k] = j
                    j += 1
            elif label_dim[label] != -1:
                dim = label_dim[label]
                operand = mindtorch.diagonal(operand, offset=0, dim1=dim, dim2=j)
                operand = mindtorch.swapaxes(operand, -1, dim)
            else:
                label_dim[label] = j
        ...
        for idx, index in enumerate(perm_shape):
            if index == -1:
                operand = mindtorch.unsqueeze(operand, -1)
                perm_shape[idx] = j
                j += 1

        operand = mindtorch.permute(operand, tuple(perm_shape))
    ...
    if has_zero_size_dim:
        ...
        return mindtorch.zeros(out_shape)
    ...
    for i in range(dim, perm_index):
        if dim_last_op[i] == 0:
            if result.shape[dim] == 1:
                result = mindtorch.squeeze(result, dim)
                dim -= 1
            else:
                result = mindtorch.sum(result, dim)
                dim -= 1
        dim += 1
...
def view_as_complex(input):
    return execute('complex', input[..., 0], input[..., 1])
...
def unfold(input, dimension, size, step):
    ...
    indices = mindtorch.Tensor(_indices)
    output = mindtorch.gather(input, _dimension, indices)
    output = mindtorch.swapaxes(output, _dimension + 1, -1)
    return output
