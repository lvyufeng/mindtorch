...
from mindtorch.distributed.checkpoint.planner import (
    TensorWriteData,
    WriteItem,
    WriteItemType,
)
...
class LocalShardsWrapper(mindtorch.Tensor):  # pyre-ignore[13]: pyre is bad at __new__
    """
    ...
    _local_shards: List[mindtorch.Tensor]
    _storage_meta: TensorStorageMetadata

    @staticmethod
    def __new__(
        cls, local_shards: List[mindtorch.Tensor], local_offsets: List[Tuple[int, ...]]
    ) -> "LocalShardsWrapper":
        ...
        r = mindtorch.Tensor._make_wrapper_subclass(  # type: ignore[attr-defined]
            cls,
            mindtorch.Size(cat_tensor_shape),
        )
    ...
    def __torch_dispatch__(cls, func, types, args=(), kwargs=None):
        ...
        dispatcher = {
            mindtorch.ops._c10d_functional.all_gather_into_tensor.default: cls.handle_all_gather_into_tensor,
            mindtorch.ops._c10d_functional.wait_tensor.default: cls.handle_wait_tensor,
    ...
    def handle_all_gather_into_tensor(args, kwargs):
        ...
        cat_tensor = mindtorch.cat(
            [t.view(-1) for t in args[0].local_shards()], dim=0
        ).view(-1, dim)
        return mindtorch.ops._c10d_functional.all_gather_into_tensor.default(
    ...
    def handle_wait_tensor(args, kwargs):
        return mindtorch.ops._c10d_functional.wait_tensor(args[0])
    ...
    def handle_clone(args, kwargs):
        ...
        if desired_memory_format and desired_memory_format != mindtorch.preserve_format:
            raise NotImplementedError(
    ...
    @property
    def device(self) -> mindtorch._C.device:  # type: ignore[override]
    ...
    def local_shards(self) -> List[mindtorch.Tensor]:
        """
        Returns a list of :class:`mindtorch.Tensor' corresponding to the
        local shards for this rank. Returns an empty list if the current rank
        does not host any shards for this Tensor.
        """
        return self._local_shards

    def local_sizes(self) -> List[mindtorch.Size]:
        """
        Returns a list of :class:`mindtorch.Size' corresponding to the
    ...
    def local_offsets(self) -> List[mindtorch.Size]:
        """
        Returns a list of :class:`mindtorch.Size' corresponding to the
    ...
    def __get_tensor_shard__(self, index: MetadataIndex) -> mindtorch.Tensor:
        """
        For compatibility with DCP, we support finding shard based on index
        Return a 'mindtorch.Tensor' shard based on 'MetadataIndex'.
        """
        # Fast lookup path
    ...
    # pyre-fixme[14]: `__repr__` overrides method defined in `mindtorch._tensor.Tensor` inconsistently.
    # pyre-fixme[3]: Return type must be annotated.
