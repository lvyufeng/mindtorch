...
class Sampler(Generic[_T_co]):
    ...
    Example:
        ...
        >>>         sizes = mindtorch.tensor([len(x) for x in self.data])
        >>>         yield from mindtorch.argsort(sizes).tolist()
        >>>
        ...
        >>>         sizes = mindtorch.tensor([len(x) for x in self.data])
        >>>         for batch in mindtorch.chunk(mindtorch.argsort(sizes), len(self)):
        >>>             yield batch.tolist()

    .. note:: The :meth:`__len__` method isn't strictly required by
              :class:`~mindtorch.utils.data.DataLoader`, but is expected in any
              calculation involving the length of a :class:`~mindtorch.utils.data.DataLoader`.
    """
    ...
    # data, e.g., `mindtorch.utils.data.Sampler`, with its subclasses optionally
...
class RandomSampler(Sampler[int]):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        if self.generator is None:
            seed = int(mindtorch.empty((), dtype=mindtorch.int64).random_().item())
            generator = mindtorch.Generator()
        ...
        if self.replacement:
            for _ in range(self.num_samples // 32):
                yield from mindtorch.randint(
                    high=n, size=(32,), dtype=mindtorch.int64, generator=generator
                ).tolist()
            yield from mindtorch.randint(
                high=n,
            ...
            for _ in range(self.num_samples // n):
                yield from mindtorch.randperm(n, generator=generator).tolist()
            yield from mindtorch.randperm(n, generator=generator).tolist()[
                : self.num_samples % n
            ]
...
class SubsetRandomSampler(Sampler[int]):
    ...
    def __iter__(self) -> Iterator[int]:
        for i in mindtorch.randperm(len(self.indices), generator=self.generator):
...
class WeightedRandomSampler(Sampler[int]):
    ...
    def __init__(
        ...
        weights_tensor = mindtorch.as_tensor(weights, dtype=mindtorch.double)
    ...
    def __iter__(self) -> Iterator[int]:
        rand_tensor = mindtorch.multinomial(
