...
import mindtorch.distributed._functional_collectives as ft_c
import mindtorch.nn.functional as F
from mindtorch import nn
from mindtorch.distributed.device_mesh import DeviceMesh
from mindtorch.distributed.tensor import distribute_module, DTensor, Replicate, Shard
from mindtorch.distributed.tensor.parallel.style import ParallelStyle
...
def _maybe_wait(tensor: mindtorch.Tensor) -> mindtorch.Tensor:
    """
...
def _partial_update(
    original: mindtorch.Tensor,
    new: mindtorch.Tensor,
    dim: int,
    n_chunks: int,
    idx: int,
    add: bool,
    ...
    if add:
        ...
        chunks[idx] = new
    return mindtorch.cat(chunks, dim=dim)
...
class _SDPAMerger:
    ...
    def __init__(self, convert_to_f32: bool, seq_dim: int):
        ...
        self._out: Optional[mindtorch.Tensor] = None
        self._lse: Optional[mindtorch.Tensor] = None
        self._convert_to_f32 = convert_to_f32
        self._out_dtype = mindtorch.float32
        self._lse_dtype = mindtorch.float32

    def _merge_one(
        self, block_out: mindtorch.Tensor, block_lse: mindtorch.Tensor, partial: bool
    ) -> None:
    ...
    def step(self, out: mindtorch.Tensor, lse: mindtorch.Tensor, partial: bool) -> None:
        self._out_dtype = out.dtype
        self._lse_dtype = lse.dtype

        if self._convert_to_f32:
            out = out.to(mindtorch.float32)
            lse = lse.to(mindtorch.float32)

        self._merge_one(out, lse, partial)

    def results(self) -> Tuple[mindtorch.Tensor, mindtorch.Tensor]:
        assert self._out is not None
        assert self._lse is not None
...
def _scaled_dot_product_ring_efficient_attention(
    ...
    key: mindtorch.Tensor,
    value: mindtorch.Tensor,
    attn_bias: Optional[mindtorch.Tensor] = None,
...
class _RingRotater(ABC):
    ...
    @abstractmethod
    def exchange_buffers(self, curr_buffer: mindtorch.Tensor) -> None:
        ...

    @abstractmethod
    def next_buffer(self) -> mindtorch.Tensor:
        ...
...
class _AllToAllRotater(_RingRotater):
    ...
    def __init__(self, pg: dist.ProcessGroup, seq_dim: int) -> None:
        self._pg = pg
        self._seq_dim = seq_dim
        self._buffer: Optional[mindtorch.Tensor] = None
...
class _AllGatherRotater(_RingRotater):
    ...
    def __init__(self, pg: dist.ProcessGroup, seq_dim: int) -> None:
        self._pg = pg
        self._seq_dim = seq_dim
        self._aggregated_buffer: Optional[mindtorch.Tensor] = None
        self._idx = 0
...
def _ring_rotate(
    block: mindtorch.Tensor, pg: dist.ProcessGroup, send_to_next: bool
) -> mindtorch.Tensor:
...
def _templated_ring_attention(
    ...
    **kwargs:
    ...
    rest: List[Any]
    out: mindtorch.Tensor
    logsumexp: mindtorch.Tensor
    ...
    for i in range(size):
        ...
        if i < (size - 1):
            ...
            next_kv = mindtorch.cat([key.flatten(), value.flatten()])
...
def _sdpa_handler(
    op_call: mindtorch._ops.OpOverload,
    args: Tuple[object, ...],
...
def _templated_ring_attention_backward(
    ...
    **kwargs: Any,
    ...
    accum_dtype = mindtorch.float32 if _cp_options.convert_to_f32 else query.dtype
    grad_query = mindtorch.zeros_like(query, dtype=accum_dtype)
    grad_key = mindtorch.zeros_like(key, dtype=accum_dtype)
    grad_value = mindtorch.zeros_like(value, dtype=accum_dtype)
    ...
    for i in range(size):
        ...
        if is_causal_behavior != _CausalBehavior.SKIP:
            ...
            grad_query_ = mindtorch.zeros_like(query, dtype=accum_dtype)
            grad_key_ = mindtorch.zeros_like(key, dtype=accum_dtype)
            grad_value_ = mindtorch.zeros_like(value, dtype=accum_dtype)
        ...
        next_grad_kv = mindtorch.cat([grad_key.flatten(), grad_value.flatten()])
...
def _scaled_dot_product_ring_flash_attention_backward(
    ...
    out: mindtorch.Tensor,
    logsumexp: mindtorch.Tensor,
    cum_seq_q: mindtorch.Tensor,
    cum_seq_k: mindtorch.Tensor,
    max_q: int,
    max_k: int,
    dropout_p: float,
    is_causal: bool,
    philox_seed: mindtorch.Tensor,
    philox_offset: mindtorch.Tensor,
    *,
    scale: Optional[float] = None,
) -> Tuple[mindtorch.Tensor, ...]:
    seq_dim = 2
...
def _distribute_function(
    ...
    Args:
        ...
        fn_module (types.ModuleType): the Python module that the function is declared.
            e.g., if ``fn`` is ``mindtorch.nn.functional.scaled_dot_product_attention``,
            ``fn_module`` is ``mindtorch.nn.functional``.
...
class _AttentionContextParallel(ParallelStyle):
    ...
    * one argument for each of: query, key, value
    ...
    def _input_fn(
        cls,
        module: nn.Module,
        inputs: Tuple[Union[mindtorch.Tensor, int, float], ...],
        device_mesh: DeviceMesh,
    ) -> Tuple[Union[mindtorch.Tensor, int, float], ...]:
        # TODO(d4l3k); this should be Shard(2), need to fix Linear layer rules
        placement = [Replicate()]

        def backward_hook(grad: mindtorch.Tensor) -> None:
        ...
        inp = []
        for input in inputs:
            if isinstance(input, mindtorch.Tensor) and not isinstance(input, DTensor):
                input = DTensor.from_local(
                    input.contiguous(), device_mesh, placement, run_check=False
                )

            if isinstance(input, mindtorch.Tensor) and input.requires_grad:
    ...
    def _output_fn(
        cls,
        module: nn.Module,
        outputs: Union[mindtorch.Tensor, Tuple[Union[mindtorch.Tensor, int, float], ...]],
        device_mesh: DeviceMesh,
    ) -> Union[
        Union[mindtorch.Tensor, int, float], Tuple[Union[mindtorch.Tensor, int, float], ...]
    ]:
        cls._CONTEXT_MANAGERS[module].__exit__(None, None, None)
        del cls._CONTEXT_MANAGERS[module]
        ...
        out = []
        for output in [outputs] if isinstance(outputs, mindtorch.Tensor) else outputs:
            output = output.to_local() if isinstance(output, DTensor) else output

            if isinstance(output, mindtorch.Tensor) and output.requires_grad:
                output.register_hook(backward_hook)

            out.append(output)

        if isinstance(outputs, mindtorch.Tensor):
            return out[0]
...
def _context_parallel(seq_dim: int, mesh: DeviceMesh) -> Generator[None, None, None]:
    ...
    def attention_input_fn(
        ...
        for arg in itertools.chain(args, kwargs.values()):
            if isinstance(arg, mindtorch.Tensor) and not isinstance(arg, DTensor):
    ...
    # wrapped fn. This is okay if users do `import mindtorch.nn.functional` but will not
    # work if users do `import mindtorch.nn.functional.scaled_dot_product_attention`.
...
class _LoadBalancer(ABC):
    ...
    @abstractmethod
    def shard(
        cls, buffer: mindtorch.Tensor, mesh: DeviceMesh, seq_dim: int
    ) -> mindtorch.Tensor:
        ...
...
class _SequentialSharder(_LoadBalancer):
    ...
    def unshard(
        ...
        all_buffers = [mindtorch.empty_like(buffer) for _ in range(mesh.size())]
        ft_c.all_gather_inplace(all_buffers, buffer, mesh)
        return mindtorch.cat(all_buffers, dim=seq_dim)
...
class _RoundRobinLoadBalancer(_LoadBalancer):
    ...
    def unshard(
        ...
        all_buffers = [mindtorch.empty_like(buffer) for _ in range(cp_world_size)]
        ft_c.all_gather_inplace(all_buffers, buffer, mesh)
        ...
        return mindtorch.cat(ordered_buffers, dim=seq_dim)


def _context_parallel_buffers(
    mesh: DeviceMesh,
    buffers: List[mindtorch.Tensor],
...
def context_parallel(
    mesh: DeviceMesh,
    *,
    buffers: Optional[List[mindtorch.Tensor]] = None,
    buffer_seq_dims: Optional[List[int]] = None,
    no_restore_buffers: Optional[Set[mindtorch.Tensor]] = None,
    ...
    (``mindtorch.nn.functional.scaled_dot_product_attention``) with the CP-enabled
    ...
    Args:
        ...
        buffers (Optional[List[mindtorch.Tensor]]): buffers that the usage depend
        ...
        no_restore_buffers (Optional[Set[mindtorch.Tensor]]): buffers in these set
    ...
    .. warning::
        `mindtorch.distributed._tensor.experimental.attention.context_parallel` is a
