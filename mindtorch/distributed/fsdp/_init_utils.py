...
import mindtorch.distributed.fsdp._exec_order_utils as exec_order_utils
import mindtorch.distributed.fsdp._traversal_utils as traversal_utils
import mindtorch.distributed.fsdp.fully_sharded_data_parallel as fsdp_file
import mindtorch.nn as nn
from mindtorch.distributed.algorithms._comm_hooks import default_hooks
from mindtorch.distributed.device_mesh import _mesh_resources, DeviceMesh
from mindtorch.distributed.distributed_c10d import _get_default_group
...
from mindtorch.distributed.fsdp._flat_param import (
    _FSDP_USE_FULL_PREC_IN_EVAL,
    FlatParameter,
    FlatParamHandle,
    HandleShardingStrategy,
)
from mindtorch.distributed.fsdp._limiter_utils import _FreeEventQueue
...
from mindtorch.distributed.fsdp.wrap import _Policy
from mindtorch.distributed.tensor.parallel.fsdp import DTensorExtensions
from mindtorch.distributed.utils import _sync_params_and_buffers
from mindtorch.utils._python_dispatch import is_traceable_wrapper_subclass
...
try:
    from mindtorchdistx import deferred_init, fake  # type: ignore[import]
except ImportError:
...
def _init_ignored_module_states(
    state: _FSDPState,
    module: nn.Module,
    ignored_modules: Optional[Iterable[mindtorch.nn.Module]],
...
def _init_device_handle(
    ...
    device_id: Optional[Union[int, mindtorch.device]],
) -> _FSDPState:
    """
    ...
    if device_id is not None:
        determined_device = (
            device_id
            if isinstance(device_id, mindtorch.device)
            else mindtorch.device(device_id)
        )
    if determined_device is None:
        ...
        determined_device = determined_device or mindtorch._C._get_accelerator()
...
def _init_buffer_state(
    ...
    _buffer_name_to_orig_dtype: Dict[str, mindtorch.dtype] = {}
...
def _init_core_state(
    ...
    if mixed_precision is not None:
        mindtorch._C._log_api_usage_once(
            f"mindtorch.distributed.fsdp.mixed_precision.{str(state.mixed_precision)}"
        )
...
def _get_ignored_modules(
    ...
    _ignored_modules: Optional[Iterable[mindtorch.nn.Module]],
) -> Set[nn.Module]:
    """
    ...
    """
    msg_prefix = "`ignored_modules` should be an iterable of `mindtorch.nn.Module`s "
    try:
    ...
    for module in ignored_root_modules:
        if not isinstance(module, mindtorch.nn.Module):
...
def _get_ignored_params(
    root_module: mindtorch.nn.Module,
    ignored_modules: Set[mindtorch.nn.Module],
    ignored_parameters: Optional[Iterable[mindtorch.nn.Parameter]] = None,
) -> Set[mindtorch.nn.Parameter]:
    """
    ...
    """
    all_ignored_params: Set[mindtorch.nn.Parameter] = set()
...
def _check_single_device_module(
    ...
    # not supported.
    if len(devices) == 2 and mindtorch.device("cpu") in devices:
...
def _get_device_from_device_id(
...
) -> Optional[mindtorch.device]:
    """
    Return a ``mindtorch.device`` for the specified ``device_id``.
    ...
    if device_id is None:
        return None
    device = (
        device_id if isinstance(device_id, mindtorch.device) else mindtorch.device(device_id)
    )
    if device.type != "cpu" and device.index is None:
        warnings.warn(
            ...
            f"If this is incorrect, please explicitly call `mindtorch.{device.type}.set_device()` "
            "before FSDP initialization or pass in the explicit device "
            "index as the `device_id` argument."
        )
        device = mindtorch.device(device_handle.current_device())
    return device
...
def _materialize_meta_module(
    root_module: nn.Module,
    device_from_device_id: Optional[mindtorch.device],
    ignored_modules: Set[nn.Module],
    device_handle: _FSDPDeviceHandle,
):
    # Run default meta device initialization
    materialization_device = device_from_device_id or mindtorch.device(
...
def _move_module_to_device(
    module: nn.Module,
    ignored_params: Set[nn.Parameter],
    ignored_buffers: Set[mindtorch.Tensor],
    device_from_device_id: Optional[mindtorch.device],
) -> None:
    """
...
def _get_compute_device(
    ...
    if param is not None and param.device.type != "cpu":
        ...
        compute_device = mindtorch.device(device_handle.current_device())
...
def _sync_module_params_and_buffers(
    ...
    been set.
    """
    module_states: List[mindtorch.Tensor] = []
...
def _check_module_states_for_sync_module_states(
    ...
    if module_states and any(
        tensor.device == mindtorch.device("cpu") for tensor in module_states
    ):
        raise ValueError(
