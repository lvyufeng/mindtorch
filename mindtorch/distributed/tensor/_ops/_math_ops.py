...
from mindtorch.distributed.tensor._utils import normalize_to_torch_size
...
class _NormPartial(Partial):
    ...
    def _pre_reduce_transform(self, tensor: mindtorch.Tensor) -> mindtorch.Tensor:
    ...
    def _post_reduce_transform(self, tensor: mindtorch.Tensor) -> mindtorch.Tensor:
        if self.reduce_op == "sum":
            assert isinstance(self.norm_type, (int, float)), f"{self.norm_type}"
            if self.norm_type != 0 and self.norm_type != 1:
...
def layer_norm_strategy(mesh: DeviceMesh, op_schema: OpSchema) -> OpStrategy:
    ...
    assert isinstance(input_strategy, OpStrategy)
    assert isinstance(normalized_shape, (int, Sequence, mindtorch.Size))
