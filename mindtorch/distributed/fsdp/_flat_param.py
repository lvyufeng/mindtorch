...
from mindtorch.distributed.utils import (
    _alloc_storage,
    _data_ptr_allocated,
    _free_storage,
    _p_assert,
)
from mindtorch.nn.parameter import _ParameterMeta  # type: ignore[attr-defined]
from mindtorch.testing._internal.distributed.fake_pg import FakeProcessGroup
...
class FlatParamShardMetadata(NamedTuple):
    ...
    Attributes:
        ...
        param_shapes (Tuple[mindtorch.Size, ...]): Parameter shapes of this rank's
            shard of the parameters; see :class:`FlatParameter`.
        param_strides (Tuple[mindtorch.Size, ...]): Parameter strides of this rank's
            shard of the parameters; see :class:`FlatParameter`.
    ...
    param_shapes: Tuple[mindtorch.Size, ...]
...
class _FlatParameterMeta(_ParameterMeta):
    ...
    def __instancecheck__(self, instance):
        ...
        return isinstance(instance, mindtorch.Tensor) and getattr(
...
class FlatParameter(nn.Parameter, metaclass=_FlatParameterMeta):
    ...
    Attributes:
        _unpadded_unsharded_size (mindtorch.Size): Unsharded flat parameter's size
        ...
        _padded_unsharded_size (mindtorch.Size): Unsharded flat parameter's size
            ...
            the all-gather.
        _sharded_size (mindtorch.Size): Sharded flat parameter's size with padding.
        ...
        _shapes (Tuple[mindtorch.Size, ...]): Each parameter's original shape.
        _strides (Tuple[mindtorch.Size, ...]): Each parameter's original stride.
        ...
        _numels_with_padding (Tuple[int, ...]): Each parameter's numel
            ...
            into the flat parameter via ``mindtorch.split()``. This may have length
    ...
    _unpadded_unsharded_size: mindtorch.Size
    _padded_unsharded_size: mindtorch.Size
    _sharded_size: mindtorch.Size
    _num_params: int
    _param_infos: Tuple[ParamInfo, ...]
    _shapes: Tuple[mindtorch.Size, ...]
...
class FlatParamHandle:
    ...
    Args:
        ...
        device (mindtorch.device): The compute and communication device, which
        ...
        mp_param_dtype (Optional[mindtorch.dtype]): Parameter mixed precision
            setting passed to the FSDP constructor.
        mp_reduce_dtype (Optional[mindtorch.dtype]): Gradient reduction mixed
    ...
    def __init__(
        ...
        offload_params: bool,
        mp_param_dtype: Optional[mindtorch.dtype],
        mp_reduce_dtype: Optional[mindtorch.dtype],
    ...
    def flatten_tensors(
        ...
        if aligned_numel > 0:
            ...
            for tensor in tensors:
                ...
                if numel_to_pad > 0 and numel_to_pad < aligned_numel:
                    ...
                    total_numel += numel_to_pad
                flat_tensors.append(
                    mindtorch.flatten(_detach_if_needed(tensor))
        ...
        return mindtorch.cat(flat_tensors, dim=0)
    ...
    def shard(self):
        ...
        if not self.uses_sharded_strategy:
            ...
            if not mindtorch.distributed._functional_collectives.is_torchdynamo_compiling():
    ...
    def _get_unpadded_shard(
        ...
        """
        chunks = (
            mindtorch.flatten(tensor).chunk(world_size)
            if _is_truly_contiguous(tensor)
    ...
    @staticmethod
    def _get_sharded_size(tensor: Tensor, rank: int, world_size: int) -> mindtorch.Size:
        """
        ...
        return mindtorch.Size([unpadded_sharded_size[0] + numel_to_pad])
    ...
    def init_flat_param_attributes(self) -> None:
        ...
        if self._offload_params:
            ...
            flat_param._cpu_grad = mindtorch.zeros_like(
        ...
        if self._uses_param_mixed_precision:
            ...
            # computation.
            flat_param._mp_shard = mindtorch.empty_like(
        ...
        if self.uses_sharded_strategy:
            ...
            flat_param._full_param_padded = mindtorch.empty(
            ...
            if self._uses_param_mixed_precision:
                ...
                flat_param._full_prec_full_param_padded = mindtorch.empty(
                    padded_unsharded_numel,
                    device=self.device,
    ...
    def _get_padded_unsharded_flat_param(self) -> mindtorch.Tensor:
        """
    ...
    def _use_unsharded_flat_param(
        self,
        padded_unsharded_flat_param: mindtorch.Tensor,
    ) -> None:
        """
    ...
    def unshard_grad(self):
        ...
        num_grad_none = mindtorch.zeros(1, dtype=mindtorch.int32, device=self.device)
        ...
        if flat_param.grad is None:
            ...
            flat_param._saved_grad_shard = None  # type: ignore[assignment]
            sharded_grad = mindtorch.zeros(flat_param._sharded_size, device=self.device)  # type: ignore[attr-defined]
        else:
        ...
        padded_unsharded_grad = mindtorch.empty(
    ...
    def to_cpu(self):
        ...
        self.flat_param_to(mindtorch.device("cpu"))
    ...
    def _get_unflat_views_unaligned(
        self,
        tensor: Optional[mindtorch.Tensor] = None,
    ) -> Iterator[Tensor]:
        """
        ...
        views = (
            ...
            for (subtensor, shape, stride, contiguous, param_extension) in zip(
                mindtorch.split(tensor, flat_param._numels, dim=0),
                flat_param._shapes,
                flat_param._strides,
    ...
    def _get_unflat_views_aligned(
        ...
        if tensor is None:
            tensor = flat_param
        splits: List[Tensor] = mindtorch.split(
    ...
    def _use_unsharded_views(self, as_params: bool) -> None:
        ...
        Note:
            ...
            annotated with `@mindtorch.no_grad()`
            `@mindtorch.enable_grad()` ensures non-empty `view.grad_fn`
        ...
        for i, (view, (param_name, module, _)) in enumerate(
            ...
            if (
                ...
                if param.grad is None:
                    param.grad = mindtorch.empty_like(param)
    ...
    def _use_sharded_views(self) -> None:
        ...
        size_0_empty_tensor = mindtorch.empty(
            0,
    ...
    def _writeback_orig_params(self) -> bool:
        ...
        for i, (
            ...
            if needs_param_writeback:
                expected_shape = mindtorch.Size([numel_in_shard])
            ...
            if param.grad is None and flat_param.grad is not None:
                ...
                if needs_grad_writeback:
                    if flat_param_grad is None:
                        flat_param_grad = mindtorch.zeros_like(flat_param)
                    expected_shape = mindtorch.Size([numel_in_shard])
                    self._writeback_tensor(
                        param.grad,
                        flat_param_grad,
    ...
    def _check_on_cpu(self, tensor: Tensor):
        _p_assert(
            tensor.device == mindtorch.device("cpu"),
...
def _convert_to_params(
    tensors: List[Union[mindtorch.Tensor, nn.Parameter]]
) -> List[nn.Parameter]:
...
def _get_aligned_numel(unsharded_dtype: mindtorch.dtype):
...
def _get_dtype_size(dtype):
    return mindtorch.empty((), dtype=dtype).element_size()


def _construct_padding_tensor(
    padding_numel: int, dtype: mindtorch.dtype, requires_grad: bool, device: mindtorch.device
):
...
def _same_storage_size(a: mindtorch.Tensor, b: int):
