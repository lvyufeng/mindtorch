...
import mindtorch
from mindtorch._ops import OpOverload
...
try:
    from mindtorch.utils._cxx_pytree import tree_leaves, tree_map_only, TreeSpec
except ImportError:
    from mindtorch.utils._pytree import (  # type: ignore[no-redef, assignment]
        tree_leaves,
        tree_map_only,
        TreeSpec,
    )
...
def _rebuild_tensor_from_dtensor_meta(arg) -> object:
    ...
    return mindtorch.empty_strided(
...
class OpSchema:
    ...
    def __post_init__(self) -> None:
        ...
        for a in self.args_schema:
            if isinstance(a, DTensorSpec) and a.tensor_meta is not None:
                if any(isinstance(s, mindtorch.SymInt) for s in a.tensor_meta.shape):
                    has_symints = True
                    break
    ...
    def return_type_tuple_tensor_like(self) -> bool:
        ...
        return len(return_types) > 1 and isinstance(
            return_types[0].type, mindtorch.TensorType
        )
    ...
    def return_type_tensor(self) -> bool:
        ...
        return isinstance(return_types[0].type, mindtorch.TensorType)
