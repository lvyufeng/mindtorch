...
from mindtorch.distributed.tensor._ops.utils import expand_to_full_mesh_op_strategy
...
def register_sharding(op: Union[OpOverload, List[OpOverload]]):
    ...
    Returns:
        ...
        as the original op (except that if an arg is a :class:`mindtorch.Tensor`, it will be
    ...
    def wrapper(custom_sharding_fn):
        def derive_schema_info(op):
            ...
            for i, arg in enumerate(op._schema.arguments):
                if isinstance(arg.type, mindtorch.IntType) or (
                    isinstance(arg.type, mindtorch.OptionalType)
                    and isinstance(arg.type.getElementType(), mindtorch.IntType)
                ):
