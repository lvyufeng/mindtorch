...
class Tensor(metaclass=TensorMeta):
    ...
    def __init__(self, *input, device=None, dtype=None, requires_grad=False, **kwargs): # pylint: disable=super-init-not-called
        ...
        if isinstance(input[0], TensorNode):
            ...
            self.tensor = MSTensor.from_numpy(input[0])
            self._user_created = True
    ...
    def __contains__(self, item):
        return mindtorch.equal(self, item).any()
    ...
    def fill_(self, value):
        mindtorch.inplace_fill(self, value)
        return self
    ...
    def set_(self, source=None, storage_offset=0, size=None, stride=None):
        if isinstance(source, (mindtorch.Storage, mindtorch.TypedStorage)):
            if size is None:
                size = source.nbytes()
            buffer = ctypes.string_at(source.data_ptr(), size)
            tensor = MSTensor.convert_bytes_to_tensor(buffer, (size // mindtorch._utils.element_size_map[self.dtype],), self.dtype)
            self.tensor.assign_value_cpp(tensor)
        return self
    ...
    # Tensor.storage
    def storage(self):
        return mindtorch.TypedStorage(wrap_storage=self.untyped_storage(), dtype=self.dtype)

    # Tensor.untyped_storage
    def untyped_storage(self):
        return mindtorch.UntypedStorage(
    ...
    def _move_to(self, device, non_blocking=False):
        if self.device == device:
            ...
            out = mindtorch.empty(self.shape, dtype=self.dtype, device=device)
            acl.rt.memcpy(out.data_ptr(), self.nbytes, self.data_ptr(), self.nbytes, MEMCPY)
            out._data.data_sync(True)
...
def tensor(data, *, dtype=None, device=None, requires_grad=False):
    ...
    tensor = Tensor(data_tensor).to(device)
