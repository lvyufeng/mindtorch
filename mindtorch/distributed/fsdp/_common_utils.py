...
import mindtorch.distributed.fsdp._flat_param as flat_param_file
import mindtorch.nn as nn
from mindtorch.distributed._composable_state import _get_module_state, _State
from mindtorch.distributed.algorithms._checkpoint.checkpoint_wrapper import (
    _CHECKPOINT_PREFIX,
)
from mindtorch.distributed.utils import _apply_to_tensors
from mindtorch.utils._mode_utils import no_dispatch
...
if TYPE_CHECKING:
    ...
    from mindtorch.distributed.fsdp._fsdp_extensions import FSDPExtensions
...
class _FSDPDeviceHandle:
    ...
    def __init__(self, device: mindtorch.device, backend: Any = None):
        if backend is None:
            try:
                ...
                raise AttributeError(
                    f"Device '{device}' does not have a corresponding backend registered as 'mindtorch.{device.type}'."
                ) from exc
        else:
            self.__backend = backend

    @classmethod
    def from_device(cls, device: mindtorch.device) -> "_FSDPDeviceHandle":
        """
        ...
        Just return mindtorch.cuda if the device is cuda to make attribute-access faster.
        Custom backend must first register a module with the same name with {device.type} on mindtorch.
        """
        if device.type == "cuda":
            return cast(_FSDPDeviceHandle, mindtorch.cuda)
        elif device.type == "mtia":
            return cast(_FSDPDeviceHandle, mindtorch.mtia)
        return cls(device)
    ...
    def __getattr__(self, __name: str) -> Any:
        try:
            ...
            raise AttributeError(
                f"Custom backend '{self.__device.type}' not implement 'mindtorch.{self.__device.type}.{__name}'"
            ) from exc
...
class _FSDPState(_State):
    def __init__(self) -> None:
        ...
        self.compute_device: Optional[mindtorch.device] = None
        ...
        self._unshard_event: Optional[mindtorch.Event] = None
...
def _set_fsdp_flattened(tensor: mindtorch.Tensor) -> None:
    """
...
def _is_fsdp_flattened(tensor: mindtorch.Tensor) -> bool:
...
def _get_param_to_fqns(
    ...
    param_to_unflat_param_names: Dict[mindtorch.nn.Parameter, List[str]] = {}
...
def _apply_to_modules(
    ...
    **kwargs,
    ...
    def f(module: mindtorch.nn.Module, prefix: str, tree_level: int, *args, **kwargs):
...
def _override_module_mixed_precision(
    ...
    for mod in root.modules():
        if isinstance(mod, module_classes_to_override):
            ...
            def cast_fn(
                dtype: mindtorch.dtype, module: nn.Module, x: mindtorch.Tensor
            ) -> mindtorch.Tensor:
                if not mindtorch.is_floating_point(x) or x.dtype == dtype:
                    return x
                _MODULE_TO_INP_DTYPE[module] = x.dtype
                return x.to(dtype)

            def forward_pre_hook(module, args):
                return _apply_to_tensors(partial(cast_fn, mindtorch.float32, module), args)
...
def _no_dispatch_record_stream(tensor: mindtorch.Tensor, stream: mindtorch.Stream) -> None:
