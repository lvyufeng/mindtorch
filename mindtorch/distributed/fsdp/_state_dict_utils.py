...
import mindtorch.distributed.algorithms._checkpoint.checkpoint_wrapper as checkpoint_wrapper
...
from mindtorch.distributed.utils import _replace_by_prefix
...
def _local_post_state_dict_hook(
    ...
    if valid_data_size > 0:
        ...
        # pickling issue (can be mindtorch.save but not mindtorch.load). Since there
    ...
    *args: Any,
    ...
    if fsdp_state._is_root:
        ...
        for key, tensor in sorted(processed_state_dict.items()):
            if key.startswith(prefix) and isinstance(tensor, mindtorch.Tensor):
