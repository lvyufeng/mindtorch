...
import mindtorch.distributed.tensor._api as dtensor
...
from mindtorch.distributed.tensor._random import is_rng_supported_mesh
from mindtorch.distributed.tensor._redistribute import redistribute_local_tensor
from mindtorch.distributed.tensor._sharding_prop import ShardingPropagator
from mindtorch.distributed.tensor._tp_conv import (
    convolution_backward_handler,
    convolution_handler,
)
from mindtorch.distributed.tensor._utils import try_find_mesh_from_args
from mindtorch.distributed.tensor.placement_types import Partial, Placement, Replicate
...
try:
    from mindtorch.utils import _cxx_pytree as pytree
except ImportError:
    from mindtorch.utils import _pytree as pytree  # type: ignore[no-redef]
...
def is_same_size_handler(
    ...
    lhs = cast(mindtorch.Tensor, args[0])
    rhs = cast(mindtorch.Tensor, args[1])
...
def found_inf_reduce_handler(
    ...
    target_tensor = cast(mindtorch.Tensor, args[1])
    spec = DTensorSpec(
        mesh=mesh,
...
class OpDispatcher:
    ...
    def __init__(self) -> None:
        ...
        # This flag is used internally to control whether we treat the mindtorch.Tensor(non-DTensor)
        ...
        if mesh.get_coordinate() is not None:
            ...
            if op_call in self._random_ops:
                ...
                first_arg, first_local_arg = cast(dtensor.DTensor, args[0]), cast(
                    mindtorch.Tensor, local_tensor_args[0]
                )
                rng_context = (
            ...
            if spec is None:
                ...
                def default_tensor(spec: DTensorSpec) -> mindtorch.Tensor:
                    if spec.tensor_meta is not None:
                        shape = spec.tensor_meta.shape
                        dtype = spec.tensor_meta.dtype
                        if len(shape) == 0:
                            # scalar tensor
                            return mindtorch.zeros((), dtype=dtype)
                        else:
                            # non-scalar tensor
                            return mindtorch.tensor([], dtype=dtype)
                    else:
    ...
    def redistribute_local_args(
        ...
        for i, arg_spec in enumerate(op_info.flat_args_schema):
            ...
            if isinstance(arg_spec, DTensorSpec):
                local_tensor = cast(mindtorch.Tensor, op_info.local_args[i])
    ...
    def unwrap_to_op_info(
        ...
        for k, v in kwargs.items():
            ...
            elif isinstance(v, mindtorch.Tensor):
                mesh = mesh or try_find_mesh_from_args(op_call, args_list)
    ...
    def wrap(res: object, spec: OutputSpecType) -> object:
        if isinstance(res, mindtorch.Tensor):
            if spec is not None:
                assert isinstance(
                    spec, DTensorSpec
    ...
    def _try_replicate_spec_for_scalar_tensor(
        ...
        if tensor_arg.numel() == 1 or self._allow_implicit_replication:
            ...
            raise RuntimeError(
                f"{op_call}: got mixed mindtorch.Tensor and DTensor, need to convert all"
                " mindtorch.Tensor to DTensor before calling distributed operators!"
            )
    ...
    def _try_replicate_dtensor_spec_in_missing_dim(
        ...
        from mindtorch.distributed.device_mesh import _mesh_resources
