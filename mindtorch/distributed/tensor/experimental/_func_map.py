...
def local_map(
    ...
    *,
    ...
    to a function that is written to be applied on ``mindtorch.Tensor`` s. It is done by extracting
    ...
    Args:
        ...
        in_placements (Tuple[`PlacementType`, ...], optional):
            ...
            argument is a :class:`mindtorch.Tensor`. In this case, the placements examination
    ...
    Example:
        ...
        >>>     partial_sum_tensor = mindtorch.mm(W, X)
        >>>     reduced_tensor = funcol.all_reduce(partial_sum_tensor, "sum", device_mesh)
        >>>     return reduced_tensor
        >>>
        >>> W = mindtorch.randn(12, 8, requires_grad=False)
        >>> X = mindtorch.randn(8, 16, requires_grad=False)
        >>> Y = mindtorch.mm(W, X)
    ...
    def wrapped(*args, **kwargs):
        ...
        for idx, arg in enumerate(flat_args):
            if isinstance(arg, DTensor):
                ...
                if in_placements is not None and not isinstance(arg, mindtorch.Tensor):
                    spec = in_placements[idx]
                    assert spec is None, (
            ...
            for out, spec in zip(flat_out, out_placements_tuple):
                if isinstance(out, mindtorch.Tensor):
                    assert not isinstance(
                        out, DTensor
                    ), f"mindtorch.Tensor output expected but received {type(out)}: {out}"
