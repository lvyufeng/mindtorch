...
from mindtorch.distributed.tensor import (
    DeviceMesh,
    distribute_module,
    distribute_tensor,
    DTensor,
...
class ColwiseParallel(ParallelStyle):
    ...
    Keyword Args:
        ...
        use_local_output (bool, optional):
            Whether to use local :class:`mindtorch.Tensor` instead of :class:`DTensor` for the module output, default: True.
    Returns:
        A :class:`ParallelStyle` object that represents Colwise sharding of the nn.Module.

    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from mindtorch.distributed.tensor.parallel import parallelize_module, ColwiseParallel
        ...
        >>> # and the output of "w1" will return :class:`mindtorch.Tensor` that shards on the last dim.
        >>>
...
class RowwiseParallel(ParallelStyle):
    ...
    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from mindtorch.distributed.tensor.parallel import parallelize_module, RowwiseParallel
        >>> from mindtorch.distributed.device_mesh import init_device_mesh
        >>> ...
        ...
        >>> # and the output of "w2" will return a replicated :class:`mindtorch.Tensor`.
        >>>
...
class SequenceParallel(ParallelStyle):
    ...
    If the input passed in to this ``nn.Module`` is a :class:`mindtorch.Tensor`, it assumes that the input is already sharded
    ...
    Keyword Args:
        ...
        use_local_output (bool, optional):
            Whether to use local :class:`mindtorch.Tensor` instead of :class:`DTensor` for the module output, default: False.
    Returns:
        A :class:`ParallelStyle` object that represents Sequence Parallel of the ``nn.Module``.

    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from mindtorch.distributed.tensor.parallel import parallelize_module, SequenceParallel
        >>> from mindtorch.distributed.device_mesh import init_device_mesh
        >>> ...
    ...
    def _replicate_module_fn(
        ...
        for p_name, param in module.named_parameters():
            ...
            replicated_param = mindtorch.nn.Parameter(
    ...
    def _prepare_input_fn(sequence_sharding, mod, inputs, device_mesh):
        ...
        if isinstance(input_tensor, DTensor):
            ...
            return input_tensor
        elif isinstance(input_tensor, mindtorch.Tensor):
            ...
            raise ValueError(
                f"expecting input of {mod} to be a mindtorch.Tensor or DTensor, but got {input_tensor}"
            )
...
class PrepareModuleInput(ParallelStyle):
    ...
    Keyword Args:
        input_layouts (Union[Placement, Tuple[Optional[Placement]]]):
            ...
            DTensors. If some inputs are not mindtorch.Tensor or no need to convert to DTensors, ``None`` need to be specified
        ...
        use_local_output (bool, optional):
            Whether to use local :class:`mindtorch.Tensor` instead of :class:`DTensor` for the module inputs, default: False.
    Returns:
        A :class:`ParallelStyle` object that prepares the sharding layouts of the nn.Module's inputs.

    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from mindtorch.distributed.tensor.parallel import parallelize_module, PrepareModuleInput
        >>> from mindtorch.distributed.device_mesh import init_device_mesh
        >>> ...
    ...
    def _prepare_input_arg(
        ...
        if input_layout is not None:
            if isinstance(input, DTensor):
                ...
                ), "expecting input to be a mindtorch.Tensor!"
...
class PrepareModuleOutput(ParallelStyle):
    ...
    Keyword Args:
        output_layouts (Union[Placement, Tuple[Placement]]):
            ...
            DTensors if they are :class:`mindtorch.Tensor`. If some outputs are not mindtorch.Tensor or no need to convert to DTensors,
        ...
        use_local_output (bool, optional):
            Whether to use local :class:`mindtorch.Tensor` instead of :class:`DTensor` for the module outputs, default: True.
    Returns:
        A ParallelStyle object that prepares the sharding layouts of the nn.Module's outputs.

    Example::
        >>> # xdoctest: +SKIP(failing)
        >>> from mindtorch.distributed.tensor.parallel import parallelize_module, PrepareModuleOutput
        >>> from mindtorch.distributed.device_mesh import init_device_mesh
        >>> ...
        >>> block = TransformerBlock(...)  # block is a nn.Module that contains an "attn" Attention submodule
        >>> tp_mesh = init_device_mesh("cuda", (8,))
