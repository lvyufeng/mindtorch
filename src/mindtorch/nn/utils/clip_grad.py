...
_tensor_or_tensors = Union[mindtorch.Tensor, Iterable[mindtorch.Tensor]]
...
def clip_grad_norm_(
        ...
        error_if_nonfinite: bool = False, foreach: Optional[bool] = None) -> mindtorch.Tensor:
    ...
    """
    if isinstance(parameters, mindtorch.Tensor):
        parameters = [parameters]
    grads = [p.grad for p in parameters if p.grad is not None]
    max_norm = float(max_norm)
    norm_type = float(norm_type)
    if len(grads) == 0:
        return mindtorch.tensor(0.)
    if norm_type == inf:
